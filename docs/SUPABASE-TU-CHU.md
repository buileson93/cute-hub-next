...
2. **Nạp schema + dữ liệu**: dùng `supabase/migrations-fresh/` cho project trống,
   hoặc gói `.zip` dump tại *Quản trị → Lưu trữ tệp → Tải .zip dump* rồi phục hồi bằng
   *Phục hồi CSDL từ gói .zip*.
...
- **Storage**: đã có chế độ *Chỉ Cloudflare R2* trong *Quản trị → Lưu trữ tệp*.
