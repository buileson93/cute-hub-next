-- Fix rpc_tai_san_toan_cuc and rpc_thanh_phan_toan_cuc to use trang_thai_id and join for labels
-- Resolves "column t.trang_thai does not exist"
...
BEGIN
  ...
  SELECT 
    jsonb_build_object(
      ...
      'nhaCungCap', ncc.ten,
      'donViQuanLy', dv.ten,
      'trangThai', tt.ten, -- Use joined label from dm_trang_thai_thiet_bi
      'viTri', v.ten,
  ...
  LEFT JOIN dm_trang_thai_thiet_bi tt ON tt.id = t.trang_thai_id; -- Join for label
END;
$function$;
...
BEGIN
  ...
  SELECT 
    jsonb_build_object(
      ...
      'taiSanTrangThai', tt.ten, -- Corrected from t.trang_thai
      'namSanXuat', t.nam_san_xuat,
      'namKhaiThac', t.nam_dua_vao_khai_thac,
      'ngayMua', to_char(t.ngay_mua, 'DD/MM/YYYY'),
      'hanBaoHanh', to_char(t.han_bao_hanh, 'DD/MM/YYYY'),
...
GRANT EXECUTE ON FUNCTION public.rpc_tai_san_toan_cuc() TO authenticated;
GRANT EXECUTE ON FUNCTION public.rpc_tai_san_toan_cuc() TO service_role;
GRANT EXECUTE ON FUNCTION public.rpc_thanh_phan_toan_cuc() TO authenticated;
GRANT EXECUTE ON FUNCTION public.rpc_thanh_phan_toan_cuc() TO service_role;