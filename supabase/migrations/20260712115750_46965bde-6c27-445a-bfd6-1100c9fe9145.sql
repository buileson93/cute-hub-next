...
BEGIN
  ...
  IF r.loai = 'move_system' THEN
    -- UI gui to_nhom_id la id PHAN LOAI dich (hierarchy PL->NH->HT->TB).
    v_to_pl := NULLIF(r.payload->>'to_nhom_id','')::uuid;
    v_to_nh_key := NULLIF(r.payload->>'to_nh_key','');
    v_to_nh_ten := NULLIF(r.payload->>'to_nh_ten','');

    -- Resolve nh_key -> nhom that thuoc phan loai dich (neu co ban ghi that).
    v_nhom_id := NULL;
    ...
    SELECT jsonb_build_object(
      ...
      'thiet_bi', (SELECT COALESCE(jsonb_agg(jsonb_build_object('id', id, 'phan_loai_id', phan_loai_id, 'nhom_he_thong_id', nhom_he_thong_id, 'linh_vuc_id', linh_vuc_id)), '[]'::jsonb)
    ...
    UPDATE public.dm_he_thong
      SET phan_loai_id = COALESCE(v_to_pl, phan_loai_id),
          nhom_he_thong_id = CASE WHEN v_to_pl IS NOT NULL THEN v_nhom_id ELSE nhom_he_thong_id END
    ...
    -- Ghi nhom hien thi (override) khi nhom la nhom suy ra chua co ban ghi that.
  ...
  ELSIF r.loai = 'move_systems' THEN
    ...
    UPDATE public.dm_he_thong
      SET phan_loai_id = COALESCE(v_to_pl, phan_loai_id),
          nhom_he_thong_id = CASE WHEN v_to_pl IS NOT NULL THEN NULL ELSE nhom_he_thong_id END
      WHERE id = ANY(v_ids);
...
BEGIN
  ...
  IF r.loai = 'move_system' THEN
    v_ht := snap->'he_thong';
    UPDATE public.dm_he_thong
      SET phan_loai_id = CASE WHEN v_ht ? 'phan_loai_id' THEN NULLIF(v_ht->>'phan_loai_id','')::uuid ELSE phan_loai_id END,
          nhom_he_thong_id = NULLIF(v_ht->>'nhom_he_thong_id','')::uuid,
          linh_vuc_id = NULLIF(v_ht->>'linh_vuc_id','')::uuid
      WHERE id = r.he_thong_id::uuid;

    UPDATE public.thiet_bi t
      SET phan_loai_id = CASE WHEN e ? 'phan_loai_id' THEN NULLIF(e->>'phan_loai_id','')::uuid ELSE t.phan_loai_id END,
  ...
  ELSIF r.loai = 'move_systems' THEN
    UPDATE public.dm_he_thong s
      SET phan_loai_id = CASE WHEN e ? 'phan_loai_id' THEN NULLIF(e->>'phan_loai_id','')::uuid ELSE s.phan_loai_id END,
          nhom_he_thong_id = NULLIF(e->>'nhom_he_thong_id','')::uuid,
          linh_vuc_id = NULLIF(e->>'linh_vuc_id','')::uuid
