...
- **`src/components/mirats/StandardTable.tsx`**
  ...
  - Replace `vw` (window width) with `containerWidth`. This ensures that when the sidebar expands/collapses, columns hide/show correctly based on the _actual space available_ to the table, not the whole window.
