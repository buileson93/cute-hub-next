...
- **Mechanism Review**: The "smart" `catValues` logic uses `matchesFilters(r, c.key)` which applies filters from all columns _except_ the one being calculated. This ensures the filter options only show values that would yield results given other active filters.
...
- **a. Pagination**: Filters will run _before_ `rowVirtualizer` or any client-side slicing. Server-side pagination is currently a concern; I will check if `pagination` prop is used for server-side fetching in known callers.
