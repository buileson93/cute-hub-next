...
*Lưu ý: Có một số tệp có số lượng không khớp do logic render động hoặc dùng `Tabs` để điều khiển state bên ngoài thay vì dùng `TabsContent` (ví dụ: `DetailLayout.tsx`), nhưng các tệp trên là lỗi hiển thị rõ rệt.*
