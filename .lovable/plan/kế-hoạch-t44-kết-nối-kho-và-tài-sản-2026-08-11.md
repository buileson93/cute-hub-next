...
2.  **Số liệu thực tế (Bước 1):**
    ...
    - _Nhận xét:_ Hệ thống hiện tại có 832 tài sản nhưng sổ kho gần như trống rỗng (chỉ có 2 giao dịch test). Người dùng chưa có thói quen nhập liệu vào sổ kho.
...
5.  **Giải pháp "Khung nhìn mới" (Bước 4):**
    ...
    - (1) _Còn mấy cái thay thế?_: `SELECT count(*) FROM thiet_bi WHERE model_id = ... AND vai_tro = 'vat_tu' AND ...` cộng với `SELECT sum(so_luong) FROM view_ton_kho WHERE model_id = ...`.
    - (2) _Dùng cho tài sản nào?_: Truy vấn ngược từ `model_id` sang các `thiet_bi` (đang ở trạng thái lắp) có cùng `model_id`.
    - (3) _Thay khi nào, từ kho nào?_: Cần nối `thiet_bi.id` hoặc `model_id` với `kho_giao_dich`. Hiện tại `kho_giao_dich` chưa có link này cho loại vật tư là `thiet_bi`.
...
_Vui lòng duyệt Giai đoạn 2 để bắt đầu thực hiện._
