...
const allFiles = getFiles(SRC_DIR).filter(f => f.endsWith('.tsx') || f.endsWith('.ts') || f.endsWith('.css'));
...
const result = {
  ...
  metrics: {
    responsivePrefixes: { sm: smCount, md: mdCount, lg: lgCount, xl: xlCount, '2xl': xxlCount, filesWithNoPrefix },
    visibility: { mdHidden, smHidden, lgHidden, hiddenWithPrefix },
    mobileLogic: { count: isMobileFiles.length, files: isMobileFiles },
