...
const DON_VI = [
  ...
  { code: "THO", ten: "Tuy Hòa" },  { code: "PCA", ten: "Phù Cát" },
  { code: "PBA", ten: "Phú Bài" },  { code: "PLK", ten: "Pleiku" },
];

type UserRow = {
  id: string; email: string; ho_ten: string | null; don_vi: string | null;
  active: boolean; created_at: string; roles: string[]; banned_until: string | null;
};
...
function AdminUsersPage() {
  ...
  if (loading) return <AppShell><div className="flex h-64 items-center justify-center"><Loader2 className="h-6 w-6 animate-spin text-muted-foreground" /></div></AppShell>;
    ...
    return (
      <AppShell>
        <div className="mx-auto max-w-lg py-20 text-center">
          ...
          <p className="mt-2 text-sm text-muted-foreground">Trang này chỉ dành cho vai trò Admin.</p>
          <Button asChild className="mt-6" variant="outline">
            <Link to="/"><ArrowLeft className="mr-2 h-4 w-4" />Về Dashboard</Link>
          </Button>
        </div>
      </AppShell>
    );
...
function AdminUsers() {
  ...
  const filtered = data.filter((u) =>
    !q || [u.email, u.ho_ten, u.don_vi, ...u.roles].some((v) => (v ?? "").toString().toLowerCase().includes(q.toLowerCase()))
  );
  ...
  type CreateInput = { email: string; password: string; ho_ten: string; don_vi: string | null; roles: AppRole[] };
  ...
  const createM = useMutation({
    ...
    onSuccess: () => { toast.success("Đã tạo tài khoản"); setCreateOpen(false); invalidate(); },
  ...
  const resetM = useMutation({
    ...
    onSuccess: () => { toast.success("Đã đặt lại mật khẩu"); setPwFor(null); },
    onError: (e: Error) => toast.error(e.message),
  });
    ...
    <div className="space-y-4">
      ...
      <Card>
        <CardHeader className="pb-3">
          <div className="flex items-center justify-between gap-3">
            <div>
              ...
              <CardDescription>{data.length} tài khoản · {data.filter((u) => u.active).length} đang hoạt động</CardDescription>
            </div>
            <div className="relative w-full max-w-xs">
              <Search className="pointer-events-none absolute left-2.5 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-muted-foreground" />
              <Input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Tìm email, tên, đơn vị…" className="h-8 pl-8 text-xs" />
            </div>
          </div>
        </CardHeader>
        <CardContent className="p-0">
          <StandardTable<UserRow>
            ...
            emptyContent={<div className="py-10 text-center text-muted-foreground text-sm">Không có tài khoản</div>}
            columns={[
              {
                key: "email", label: "Email · Họ tên", filter: "text",
              ...
              },
              {
                ...
                cell: (u) => (
                  <div className="flex justify-end gap-1">
                    ...
                    <Button size="sm" variant="outline" onClick={() => setPwFor(u)}><KeyRound className="h-3.5 w-3.5" /></Button>
      ...
      {editing && (
        <UserForm
          ...
          onSubmit={(v) => updateM.mutate({
            user_id: editing.id, ho_ten: v.ho_ten, don_vi: v.don_vi, roles: v.roles,
          })}
          mode="edit"
        />
      )}
...
function UserForm({
  open, onClose, title, initial, onSubmit, submitting, mode,
}: {
  open: boolean; onClose: () => void; title: string; submitting: boolean;
  mode: "create" | "edit";
  initial?: { email: string; ho_ten: string; don_vi: string | null; roles: AppRole[] };
  onSubmit: (v: { email: string; password: string; ho_ten: string; don_vi: string | null; roles: AppRole[] }) => void;
}) {
  ...
  function submit() {
    if (mode === "create" && (!email || !password)) { toast.error("Nhập email và mật khẩu"); return; }
    if (!hoTen.trim()) { toast.error("Nhập họ tên"); return; }
    if (roles.length === 0) { toast.error("Chọn ít nhất 1 vai trò"); return; }
    ...
    <Dialog open={open} onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            {mode === "create" ? "Người dùng sẽ đăng nhập bằng email và mật khẩu do bạn cấp." : "Cập nhật vai trò và đơn vị. Email không đổi được."}
          </DialogDescription>
        </DialogHeader>
        <div className="space-y-3">
          ...
          {mode === "create" && (
            <div className="space-y-1.5">
              ...
              <Input value={password} onChange={(e) => setPassword(e.target.value)} type="text" placeholder="Ít nhất 6 ký tự" />
            </div>
          )}
          <div className="space-y-1.5">
            <Label>Họ tên</Label>
          ...
          <div className="space-y-1.5">
            <Label>Đơn vị</Label>
            <Select value={donVi ?? "__none"} onValueChange={(v) => setDonVi(v === "__none" ? null : v)}>
              <SelectTrigger><SelectValue placeholder="Chọn đơn vị" /></SelectTrigger>
              <SelectContent>
                <SelectItem value="__none">— Không gán —</SelectItem>
                {DON_VI.map((d) => <SelectItem key={d.code} value={d.code}>{d.code} · {d.ten}</SelectItem>)}
              </SelectContent>
            </Select>
          </div>
          <div className="space-y-1.5">
            ...
            <div className="grid grid-cols-2 gap-2">
              {ROLES.map((r) => (
                <label key={r.value} className="flex items-center gap-2 rounded-md border p-2 text-xs">
                  <Checkbox
...
function ResetPwDialog({ user, onSubmit, onClose, submitting }: {
  user: UserRow; onSubmit: (pw: string) => void; onClose: () => void; submitting: boolean;
}) {
    ...
    <Dialog open onOpenChange={(v) => !v && onClose()}>
      <DialogContent className="sm:max-w-sm">
        <DialogHeader>
          ...
          <DialogDescription>Cho tài khoản <span className="font-mono">{user.email}</span></DialogDescription>
        </DialogHeader>
        <div className="space-y-1.5">
          <Label>Mật khẩu mới</Label>
          <Input value={pw} onChange={(e) => setPw(e.target.value)} type="text" placeholder="Ít nhất 6 ký tự" />
        </div>
        <DialogFooter>
          <Button variant="outline" onClick={onClose}>Huỷ</Button>
          <Button onClick={() => { if (pw.length < 6) { toast.error("Ít nhất 6 ký tự"); return; } onSubmit(pw); }} disabled={submitting}>
