  ...
  head: () => ({
    meta: [
      { title: "Trung tâm rà soát dữ liệu — MIRATS" },
      {
        name: "description",
        content:
          "Rà soát chất lượng dữ liệu nhập: trùng lặp, xung đột tham chiếu, thiếu dữ liệu và danh mục gần trùng.",
      },
    ],
  }),
...
function severityBadge(sev: ReviewSeverity) {
  const variant =
    sev === "error" ? "destructive" : sev === "needs_review" ? "secondary" : "outline";
...
function ReviewConsole() {
  ...
  const [donViOptions, setDonViOptions] = useState<ComboOption[]>([
    { value: "", label: "Tất cả đơn vị" },
  ]);
  const [batchOptions, setBatchOptions] = useState<ComboOption[]>([
  ...
  const run = async () => {
    ...
    try {
      ...
      toast.success(
        `Đã rà soát ${res.metrics.total} dòng của ${res.batchCount} lô (${res.elapsedMs} ms)`,
      );
    } catch (e: any) {
    ...
    () => [
      {
        ...
        label: "Mức",
        minW: "min-w-[110px]",
        value: (r) => SEVERITY_LABELS[r.severity],
        cell: (r) => severityBadge(r.severity),
        filter: "cat",
      },
      {
        key: "entityLabel",
      ...
      },
      {
        ...
        value: (r) => r.keyValue,
        sticky: true,
        cell: (r) => <span className="font-medium">{r.keyValue || "—"}</span>,
      },
      {
        key: "displayName",
        label: "Tên",
      ...
      { key: "reason", label: "Chẩn đoán", minW: "min-w-[260px]", value: (r) => r.reason },
      {
        ...
        label: "Ứng viên khớp",
        minW: "min-w-[160px]",
        value: (r) => r.candidateLabel ?? "",
        cell: (r) =>
          r.candidateCount > 0 ? (
            <span>
              {r.candidateLabel ?? "?"}
              {r.candidateCount > 1 ? ` (+${r.candidateCount - 1})` : ""}
            </span>
          ) : (
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        help={
          <>
            Rà soát các lô nhập chưa xử lý: có thể trùng, trùng serial, xung đột tham chiếu, thiếu
            dữ liệu, danh mục gần trùng. Chỉ xem &amp; phân loại — không tự sửa.
          </>
        }
        actions={
          <Button asChild variant="outline">
            <Link to="/admin/nhap-lieu">
              Mở Nhập/Xuất hàng loạt <ExternalLink className="ml-1 h-4 w-4" />
            </Link>
          </Button>
        }
      />
      ...
      <Card>
        <CardHeader className="pb-3">
          ...
          <CardDescription>
            Lọc theo loại dữ liệu, đơn vị, hệ thống rồi rà soát lại.
          </CardDescription>
        </CardHeader>
        <CardContent className="grid gap-3 md:grid-cols-3 lg:grid-cols-5">
          ...
          <div className="space-y-1">
            ...
            <Input
              value={heThong}
              onChange={(e) => setHeThong(e.target.value)}
      ...
      {m && (
        <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
          <MetricCard
            ...
            label="An toàn (auto)"
            value={`${m.autoSafe}`}
            sub={`${(m.autoSafeRate * 100).toFixed(1)}%`}
          ...
          <MetricCard
            ...
            label="Cần rà soát"
            value={`${m.needsReview}`}
            sub={`${(m.needsReviewRate * 100).toFixed(1)}%`}
            tone="text-amber-600"
          />
          <MetricCard
            icon={FileWarning}
            label="Lỗi"
            value={`${m.errors}`}
            sub={`${(m.errorRate * 100).toFixed(1)}%`}
            tone="text-destructive"
          />
          <MetricCard
            icon={RefreshCw}
            label="Lô đã hoàn tác"
            value={`${data?.rolledBackCount ?? 0}`}
            sub={`${data?.batchCount ?? 0} lô đang mở`}
            tone="text-muted-foreground"
          />
          <MetricCard
            icon={Timer}
            label="Thời gian xử lý"
            value={`${data?.elapsedMs ?? 0} ms`}
            sub={`${m.total} dòng`}
            tone="text-muted-foreground"
          />
        </div>
