...
function BanQuyenDetailView() {
  ...
  if (isLoading) return <div className="p-8 text-center text-muted-foreground">Đang tải chi tiết bản quyền…</div>;
  if (!bq) return <div className="p-8 text-center text-red-500 font-bold">Không tìm thấy bản quyền mã {ma}</div>;
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        <Button variant="ghost" size="icon" asChild className="rounded-full">
          <Link to="/phan-mem-ban-quyen"><ArrowLeft className="h-5 w-5" /></Link>
        </Button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <KpiCard icon={ShieldCheck} label="Số ghế sử dụng" value={`${bq.gheDaDung}/${bq.so_ghe ?? "∞"}`} subValue={`${seatsPct}% công suất`} />
        <KpiCard icon={Clock} label="Thời hạn còn lại" value={bq.soNgayConLai !== null ? `${bq.soNgayConLai} ngày` : "Vĩnh viễn"} subValue={bq.ngay_het_han || "Không thời hạn"} />
        <KpiCard icon={Wallet} label="Giá trị" value={dinhDangTien(bq.gia_tri)} subValue={`Hợp đồng: ${bq.so_hop_dong || "—"}`} />
        <KpiCard icon={Tag} label="Phân loại" value={bq.loaiTen || "—"} subValue={bq.nha_phat_hanh || "—"} />
      </div>

      <Tabs defaultValue="overview" className="w-full">
        <TabsList className="bg-muted/50 p-1">
          <TabsTrigger value="overview" className="gap-2"><Info className="h-4 w-4" /> Tổng quan</TabsTrigger>
          <TabsTrigger value="assignment" className="gap-2"><Laptop className="h-4 w-4" /> Cấp phát ({bq.gheDaDung})</TabsTrigger>
          <TabsTrigger value="files" className="gap-2"><FileText className="h-4 w-4" /> Tệp đính kèm ({teps.length})</TabsTrigger>
          <TabsTrigger value="history" className="gap-2"><History className="h-4 w-4" /> Nhật ký</TabsTrigger>
        </TabsList>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <Card className="lg:col-span-2">
              ...
              <CardContent className="grid grid-cols-1 sm:grid-cols-2 gap-y-4 gap-x-8">
                ...
                {bq.ghi_chu && (
                  <div className="sm:col-span-2 pt-2 border-t mt-2">
                    <span className="text-xs font-bold uppercase text-muted-foreground block mb-1">Ghi chú</span>
                    <p className="text-sm text-muted-foreground whitespace-pre-wrap">{bq.ghi_chu}</p>
                  </div>
                )}
              </CardContent>
            </Card>
              ...
              <Card>
                ...
                <CardContent className="space-y-4">
                  <div className="flex justify-between items-end">
                    <span className="text-2xl font-black tabular-nums">{bq.gheDaDung} <span className="text-sm font-normal text-muted-foreground">/ {bq.so_ghe ?? "∞"}</span></span>
            ...
            <CardContent className="p-0">
              <StandardTable
                ...
                columns={[
                  ...
                  { key: "ngay_thu_hoi", label: "Ngày thu hồi", value: (r: any) => r.ngay_thu_hoi || "—" },
                  {
                    key: "actions",
                    label: "Thao tác",
                    align: "right",
            ...
            <CardContent className="p-0">
              <StandardTable
                ...
                columns={[
                  ...
                  { key: "created_at", label: "Ngày tải lên", value: (r: any) => new Date(r.created_at).toLocaleDateString("vi-VN") },
                  {
                    key: "actions",
                    label: "Thao tác",
                    align: "right",
      ...
      <BanQuyenCapPhatDialog open={capPhatOpen} onOpenChange={setCapPhatOpen} banQuyen={bq} canManage={canManage} />
...
function KpiCard({ icon: Icon, label, value, subValue }: { icon: any; label: string; value: string; subValue?: string }) {
  return (
