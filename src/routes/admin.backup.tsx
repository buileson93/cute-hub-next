...
export const Route = createFileRoute("/admin/backup")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Sao lưu toàn bộ cơ sở dữ liệu, đồng bộ lên đám mây và khôi phục dữ liệu an toàn.",
      },
...
function BackupPage() {
  ...
  const [prog, setProg] = useState<{ running: boolean; pct: number; msg: string }>({
    running: false,
    pct: 0,
    msg: "",
  });
    ...
    while (true) {
      ...
      while ((idx = buf.indexOf("\n")) >= 0) {
        ...
        if (obj.error) errMsg = obj.error;
          ...
          final = obj;
          setProg((p) => ({ ...p, pct: 100, msg: "Hoàn tất" }));
  ...
  const doBackup = useMutation({
    ...
    onSuccess: (res: any) => {
      ...
      Object.entries(db).forEach(([_k, v]: any) => {
        if (v.ok) toast.success(v.msg);
        else toast.warning(v.msg);
      });
      qc.invalidateQueries({ queryKey: ["backups"] });
      clearProgSoon();
    },
    onError: (e: any) => {
      toast.error(e.message ?? "Sao lưu lỗi");
      setProg({ running: false, pct: 0, msg: "" });
    },
  });
  ...
  const doBackupDownload = useMutation({
    ...
    onError: (e: any) => {
      toast.error(e.message ?? "Sao lưu & tải về lỗi");
      setProg({ running: false, pct: 0, msg: "" });
    },
  });
  ...
  const doRestore = useMutation({
    ...
    onError: (e: any) => {
      toast.error(e.message ?? "Khôi phục lỗi");
      setConfirmRestore(null);
    },
  });
      ...
      <AppShell>
        <div className="mx-auto mt-16 max-w-md text-center">
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Tính năng sao lưu & khôi phục dành riêng cho quản trị viên.
          </p>
        </div>
      </AppShell>
    );
    ...
    <AppShell>
      <div className="mx-auto max-w-5xl space-y-6">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Tạo gói sao lưu <b>.zip</b> chứa <b>toàn bộ dữ liệu</b>: mọi bảng (kể cả tài khoản,
              phân quyền, nhật ký), tệp <code>database.sql</code> (khôi phục được),{" "}
              <code>data.json</code> và <b>toàn bộ tài liệu &amp; hình ảnh</b> trong Storage — rồi
              đồng bộ lên các đích đã chọn.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="grid gap-3 sm:grid-cols-3">
              <DestCard
                icon={Cloud}
                label="Đám mây nội bộ"
                desc="Luôn bật · lưu an toàn"
                active
                checked
                disabled
              />
              <DestCard
                ...
                onToggle={() =>
                  c.gdrive
                    ? setDestGdrive((v) => !v)
                    : toast.info("Vào Kết nối để bật Google Drive")
                }
              />
              <DestCard
                icon={Cloud}
                ...
                checked={destS3}
                onToggle={() =>
                  c.s3 ? setDestS3((v) => !v) : toast.info("Vào Kết nối để bật Amazon S3")
                }
              />
            </div>
            <div className="flex flex-wrap items-center gap-3">
              <Button
                ...
                disabled={doBackup.isPending || doBackupDownload.isPending}
              >
                {doBackup.isPending ? (
              ...
              <Button
                variant="outline"
                onClick={() => doBackupDownload.mutate()}
                disabled={doBackup.isPending || doBackupDownload.isPending}
              >
                {doBackupDownload.isPending ? (
                  <Loader2 className="mr-2 h-4 w-4 animate-spin" />
                ) : (
            ...
            {prog.running && (
              <div className="space-y-2 rounded-lg border border-primary/20 bg-primary/5 p-3">
                ...
                <p className="text-[11px] text-muted-foreground">
                  Đang sao lưu <b>toàn bộ</b>: mọi bảng (kể cả tài khoản, phân quyền, nhật ký), tài
                  liệu và hình ảnh trong Storage.
                </p>
              </div>
            )}
            <div className="flex items-start gap-2 rounded-lg border border-amber-200 bg-amber-50 p-3 text-xs text-amber-800">
              ...
              <span>
                <b>SFTP / SMB v3:</b> hạ tầng chạy serverless nên không kết nối trực tiếp được. Hãy
                tải bản backup về máy (nút <Download className="inline h-3 w-3" /> bên dưới) rồi đẩy
                lên SFTP/SMB bằng một tác vụ tại chỗ, hoặc dùng Google Drive / S3.
              </span>
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Tải lên tệp backup <b>.zip</b> (gói đầy đủ), <b>.json.gz</b> hoặc <b>.json</b> để khôi
              phục dữ liệu (chỉ áp dụng cho các bảng nghiệp vụ, bỏ qua tài khoản & phân quyền).
            </CardDescription>
          </CardHeader>
          <CardContent>
            <input
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              100 bản backup gần nhất. Có thể tải xuống hoặc khôi phục.
            </CardDescription>
          </CardHeader>
          <CardContent>
            {backups.isLoading ? (
              ...
              <p className="py-8 text-center text-sm text-muted-foreground">
                Chưa có bản backup nào. Bấm “Sao lưu ngay” để tạo bản đầu tiên.
              </p>
            ) : (
                      ...
                      <TableRow key={b.id}>
                        ...
                        <TableCell>
                          <div className="flex justify-end gap-1">
                            <IconBtn
                              tip="Tải xuống"
                              onClick={() => doDownload.mutate(b.id)}
                              loading={doDownload.isPending && doDownload.variables === b.id}
                            >
                            ...
                            <IconBtn
                              tip="Khôi phục"
                              onClick={() =>
                                setConfirmRestore({ id: b.id, label: fmtDate(b.created_at) })
                              }
                            >
                            ...
                            <IconBtn
                              tip="Xoá"
                              onClick={() => doDelete.mutate(b.id)}
                              loading={doDelete.isPending && doDelete.variables === b.id}
                            >
        ...
        <DialogContent>
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              <RotateCcw className="h-5 w-5 text-amber-600" /> Xác nhận khôi phục
            </DialogTitle>
            <DialogDescription>
              Khôi phục dữ liệu từ bản backup <b>{confirmRestore?.label}</b>. Thao tác này sẽ{" "}
              <b>ghi đè toàn bộ dữ liệu hiện tại</b> của các bảng nghiệp vụ. Tài khoản, phân quyền
              và nhật ký sẽ được giữ nguyên. Không thể hoàn tác.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            ...
            <Button
              ...
              onClick={() => confirmRestore && doRestore.mutate(confirmRestore.id)}
      ...
      <Dialog open={!!confirmUpload} onOpenChange={(o) => !o && setConfirmUpload(null)}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              <Upload className="h-5 w-5 text-amber-600" /> Khôi phục từ tệp tải lên
            </DialogTitle>
            <DialogDescription>
              Khôi phục từ tệp <b>{confirmUpload?.filename}</b>. Thao tác này sẽ{" "}
              <b>ghi đè dữ liệu hiện tại</b> bằng nội dung trong tệp. Tài khoản & phân quyền được
              giữ nguyên. Không thể hoàn tác.
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <Button variant="outline" onClick={() => setConfirmUpload(null)}>
              Huỷ
            </Button>
            <Button
              className="bg-amber-600 hover:bg-amber-700"
              onClick={() => confirmUpload && doRestoreUpload.mutate(confirmUpload)}
              disabled={doRestoreUpload.isPending}
            >
              {doRestoreUpload.isPending && <Loader2 className="mr-2 h-4 w-4 animate-spin" />} Khôi
              phục
            </Button>
          </DialogFooter>
        </DialogContent>
  ...
  return (
    ...
    >
      ...
      <div className="min-w-0">
        ...
        <div className={`text-xs ${active ? "text-emerald-600" : "text-muted-foreground"}`}>
          {desc}
        </div>
      </div>
    </button>
