    ...
    return (
      <div className="mx-auto max-w-2xl p-8 text-center">
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          Thành phần có thể đã bị xoá hoặc bạn không có quyền xem.
        </p>
        <Button asChild variant="outline" className="mt-4">
          <Link to="/he-thong/$id" params={{ id: heThongId }}>
            Quay lại sổ hệ thống
          </Link>
        </Button>
      </div>
    );
  ...
  return (
    <div className="mx-auto max-w-6xl flex flex-col gap-4 p-4">
      ...
      <div className="flex flex-wrap gap-2 mb-2">
        ...
        {tp.dm_loai_thiet_bi?.ten && (
          <Badge variant="secondary" className="font-normal">
            Loại: {tp.dm_loai_thiet_bi.ten}
          </Badge>
        )}
        {tp.dm_vi_tri?.ten && (
          <Badge variant="secondary" className="font-normal">
            Vị trí: {tp.dm_vi_tri.ten}
          </Badge>
        )}
      </div>
      ...
      <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
        <KpiCell
          ...
          value={loadingKpi ? "…" : String(kpi?.so_su_co_12m ?? 0)}
          hint={kpi?.so_su_co_mo ? `${kpi.so_su_co_mo} đang mở` : "Không có sự cố mở"}
          tone={kpi?.so_su_co_mo ? "warn" : "ok"}
        />
        <KpiCell
          icon={Wrench}
          label="Bảo dưỡng (12 tháng)"
          value={loadingKpi ? "…" : String(kpi?.so_bao_tri_12m ?? 0)}
          hint={kpi?.ti_le_dat != null ? `${Math.round(kpi.ti_le_dat)}% Đạt` : "Chưa có kết quả"}
        />
        <KpiCell
          icon={Gauge}
          label="MTBF (ngày)"
          value={fmtNum(kpi?.mtbf_days)}
          hint="Trung bình giữa các sự cố"
        />
        <KpiCell
          icon={TrendingUp}
          label="MTTR (giờ)"
          value={fmtNum(kpi?.mttr_hours)}
          hint="Trung bình khắc phục"
        />
      </div>
      ...
      {chartData.length > 0 && (
        <Card>
          <CardHeader className="hidden">
            <CardTitle className="text-sm">Sự cố theo tháng (12 tháng gần nhất)</CardTitle>
          </CardHeader>
      ...
      <Tabs defaultValue="tai-san" className="space-y-3">
        <TabsList>
          <TabsTrigger value="tai-san" className="gap-1.5">
            <HardDrive className="h-3.5 w-3.5" /> Tài sản gắn
          </TabsTrigger>
          <TabsTrigger value="nhat-ky" className="gap-1.5">
            <Activity className="h-3.5 w-3.5" /> Nhật ký sự kiện
          </TabsTrigger>
        </TabsList>
                ...
                <div className="rounded-md border p-3">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <Badge variant="outline" className="gap-1">
                      <CheckCircle2 className="h-3 w-3 text-emerald-600" /> Đang lắp
                    </Badge>
                  </div>
                  {dangLap.ma_serial && (
                  ...
                  <div className="mt-1 text-xs text-muted-foreground">
                    Lắp từ:{" "}
                    {dangLap.tu_ngay ? new Date(dangLap.tu_ngay).toLocaleDateString("vi-VN") : "—"}
                  ...
                  <div className="mt-3">
                    <Button asChild size="sm" variant="outline">
                      <Link
                        ...
                        params={{ maThietBi: dangLap.ma_thiet_bi }}
                  ...
                  <p className="mt-3 text-xs text-muted-foreground">
                    Để <strong>tháo / thay thế / điều chuyển</strong> tài sản: vào cây hệ thống →
                    chọn thành phần → mở sidebar chi tiết.
                  </p>
                </div>
              ) : (
                <p className="text-sm text-muted-foreground">
                  Chưa có tài sản lắp vào thành phần này.
                </p>
              )}
            </CardContent>
          </Card>
                  ...
                  {history.map((h) => {
                    ...
                    return (
                      <li key={h.gan_id} className="relative mb-4 last:mb-0">
                        <span
                          className={`absolute -left-[31px] flex h-6 w-6 items-center justify-center rounded-full ring-4 ring-background ${dot}`}
                        >
                        ...
                        <div className="rounded-md border p-3 text-sm">
                          <div className="flex flex-wrap items-center gap-2">
                            ...
                            <Link
                              to="/thiet-bi/$maThietBi"
                              params={{ maThietBi: h.ma_thiet_bi }}
                              search={{ tab: "tong-quan", doc: undefined, q: undefined }}
                          ...
                          <div className="mt-0.5 text-xs text-muted-foreground">
                            ...
                            {" → "}
                            {h.den_ngay
                              ? new Date(h.den_ngay).toLocaleDateString("vi-VN")
                              : "hiện tại"}
                            {h.ly_do && <> · {h.ly_do}</>}
                          </div>
                        </div>
          ...
          <Card>
            ...
            <CardContent className="pt-0">
              <LyLichThanhPhanPanel
                thanhPhanId={tpId}
                empty="Chưa có sự kiện cho thành phần này."
              />
            </CardContent>
          </Card>
        </TabsContent>
      ...
      <Sheet open={quick !== null} onOpenChange={(o) => !o && setQuick(null)}>
        <SheetContent side="right" className="w-full overflow-y-auto sm:max-w-2xl">
          <SheetHeader>
            ...
            <SheetDescription>
              Tác nghiệp cho thành phần: <strong>{tp.ten}</strong>
            </SheetDescription>
          </SheetHeader>
          <div className="mt-4">
            {quick === "su_co" && (
              <SuCoMoiForm defaultHeThongId={heThongId} embedded onDone={() => setQuick(null)} />
            )}
            {quick === "bao_tri" && (
              <BaoTriMoiForm defaultHeThongId={heThongId} embedded onDone={() => setQuick(null)} />
            )}
            {quick === "hong_hoc" && (
              <HongHocMoiForm defaultHeThongId={heThongId} embedded onDone={() => setQuick(null)} />
            )}
          </div>
        </SheetContent>
      </Sheet>
