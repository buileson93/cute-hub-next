  ...
  head: () => ({
    meta: [
      { title: "Sơ đồ đấu nối — Tài sản MIRATS" },
      {
        name: "description",
        content:
          "Quản lý kết nối vật lý/logic giữa tài sản: cổng, cáp, mạch — nguồn dữ liệu chuẩn.",
      },
    ],
  }),
...
function TopologyPage() {
  ...
  async function handleAdd() {
    if (!tuId || !denId) {
      toast.error("Chọn đủ tài sản nguồn và đích");
      return;
    }
    if (tuId === denId) {
      toast.error("Hai đầu kết nối phải khác nhau");
      return;
    }
    try {
      await addMut.mutateAsync({
        tu_thiet_bi_id: tuId,
        den_thiet_bi_id: denId,
        tu_cong: tuCong,
        den_cong: denCong,
        loai,
        ten_mach: tenMach,
  ...
  async function handleImport() {
    ...
    try {
      ...
      setImpReport(rep);
      toast.success(
        `Nhập xong: tạo mới ${rep.created}, khớp ${rep.mapped}, chưa khớp ${rep.unmapped}`,
      );
    } catch (e: unknown) {
  ...
  return (
    <div className="space-y-6">
            ...
            >
              ...
              <DialogContent>
                <DialogHeader>
                  ...
                  <DialogDescription>
                    Hệ thống đọc các đường nối trong sơ đồ đã chọn và tạo kết nối chuẩn cho những
                    đầu nối trỏ đúng tới tài sản trong CSDL. Chạy lại nhiều lần an toàn (không tạo
                    trùng).
                  </DialogDescription>
                </DialogHeader>
                  ...
                  {impReport && (
                    <div className="rounded-md border bg-muted/40 p-3 text-sm">
                      <div className="flex flex-wrap gap-3">
                        <span>
                          Tạo mới: <b>{impReport.created}</b>
                        </span>
                        <span>
                          Đã khớp sẵn: <b>{impReport.mapped}</b>
                        </span>
                ...
                <div className="space-y-3">
                  <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                    ...
                    <div className="space-y-1.5">
                      ...
                      <Input
                        value={tuCong}
                        onChange={(e) => setTuCong(e.target.value)}
                        placeholder="VD: Port 1"
                      />
                    </div>
                    <div className="space-y-1.5">
                      ...
                      <Input
                        value={denCong}
                        onChange={(e) => setDenCong(e.target.value)}
                        placeholder="VD: eth0"
                      />
                    </div>
                  </div>
                  ...
                  <div className="space-y-1.5">
                    ...
                    <Input
                      value={tenMach}
                      onChange={(e) => setTenMach(e.target.value)}
                      placeholder="VD: Mạch VHF-01"
                    />
                  </div>
                  <div className="space-y-1.5">
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        ...
        <StatCard
          label="Cáp vật lý"
          value={ketNoi.filter((k) => k.loai === "CAP").length}
          icon={Cable}
        />
        <StatCard
          label="Mạch/Circuit"
          value={ketNoi.filter((k) => k.loai === "MACH").length}
          icon={Waypoints}
        />
        <StatCard
          label="Đơn vị liên quan"
          value={new Set(ketNoi.map((k) => k.don_vi_ma).filter(Boolean)).size}
          icon={Building2}
        />
      </div>
                                ...
                                <AlertDialogAction
                                  onClick={async () => {
                                    try {
                                      ...
                                      toast.error(
                                        e instanceof Error ? e.message : "Không xoá được",
                                      );
                                    }
                                  }}
                                >
