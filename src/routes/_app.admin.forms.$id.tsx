...
import {
  saveChecklistDesigner,
  validateChecklist,
  hasBlocking as hasChkBlocking,
...
export const Route = createFileRoute("/_app/admin/forms/$id")({
  head: () => ({
    meta: [{ title: "Thiết kế mẫu — MIRATS" }, { name: "robots", content: "noindex, nofollow" }],
  }),
...
function FormEditor() {
  ...
  const { data: taxo } = useQuery({
    ...
    enabled: canManage,
    queryFn: async () => {
      const { data, error } = await supabase.from("dm_he_thong").select("id, ma, ten").order("ten");
  ...
  useEffect(() => {
    if (tpl && fields !== null && !hydratedRef.current) {
  ...
  if (!canManage) {
    return (
      <div className="p-16 text-center">
        ...
        <p className="mt-3">Không có quyền.</p>
      </div>
    );
  }
  ...
  const saveChecklist = async () => {
    ...
    if (chkBlocked) {
      toast.error("Bảng kiểm bảo dưỡng chưa hợp lệ — sửa lỗi trước khi lưu.");
      return;
    }
    try {
      setChkSaving(true);
  ...
  return (
    <div className="flex h-[calc(100vh-3rem)] flex-col">
      ...
      <div className="flex flex-wrap items-center gap-2 border-b px-4 py-2">
        ...
        <div className="ml-auto flex items-center gap-1">
          ...
          <Button
            ...
            className="h-8"
            onClick={() =>
              downloadBundleJson(buildBundle(tpl, fields, linkedHt ?? []), tpl.ten || "template")
            }
          >
          ...
          <Tabs value={tab} onValueChange={(v) => setTab(v as typeof tab)}>
            <TabsList className="h-8">
              ...
              <TabsTrigger value="info" className="h-6 text-xs">
                <FileText className="mr-1 h-3 w-3" />
                Thông tin
              </TabsTrigger>
              <TabsTrigger value="includes" className="h-6 text-xs">
          ...
          {tab === "checklist" ? (
            <Button
              ...
              disabled={chkSaving || chkBlocked || !chkDirty}
            >
              {chkSaving ? (
      ...
      {issues.length > 0 && (
        <div className="max-h-32 overflow-y-auto border-b bg-amber-50/60 px-4 py-2 text-xs dark:bg-amber-950/20">
          <ul className="space-y-0.5">
            {issues.slice(0, 6).map((iss, k) => (
              ...
              >
                ...
                {iss.field_index != null && (
                  <span className="font-mono">#{iss.field_index + 1} </span>
                )}
                {iss.message}
                {iss.field_index != null && (
                  <button
                    className="ml-2 underline"
                    onClick={() => setSelectedIdx(iss.field_index!)}
                  >
                    Đi tới
                  </button>
                )}
            ...
            {issues.length > 6 && (
              <li className="text-muted-foreground">… và {issues.length - 6} mục nữa</li>
            )}
          </ul>
        </div>
      )}
      ...
      {tab === "design" &&
        ...
        fields.length === 0 &&
        chkSections &&
        chkSections.some((s) => (s.items?.length ?? 0) > 0) && (
          <div className="border-b bg-sky-50 px-4 py-2 text-xs text-sky-800 dark:bg-sky-950/30 dark:text-sky-200">
            <AlertTriangle className="mr-1 inline h-3 w-3" />
            Mẫu này (AWOS/PL-KT) dùng <b>Bảng kiểm bảo dưỡng</b> — không có "Trường" ở tab Biểu mẫu
            tự do.
            <button
              className="ml-2 rounded bg-sky-600 px-2 py-0.5 text-white hover:bg-sky-700"
      ...
      <div className="flex-1 min-h-0">
            ...
            <>
              {fields.length === 0 &&
                ...
                chkSections.some((s) => (s.items?.length ?? 0) > 0) && (
                  <div className="border-b bg-sky-50 px-4 py-2 text-xs text-sky-800 dark:bg-sky-950/30 dark:text-sky-200">
                    <AlertTriangle className="mr-1 inline h-3 w-3" />
                    Mẫu này (AWOS/PL-KT) dùng <b>Bảng kiểm bảo dưỡng</b> — không có "Trường" ở tab
                    Biểu mẫu tự do.
                    <button
                      className="ml-2 rounded bg-sky-600 px-2 py-0.5 text-white hover:bg-sky-700"
                      onClick={() => setTab("checklist")}
                    ...
                    <div className="mx-auto max-w-3xl">
                      <div className="mb-4 text-center">
                        <h1 className="text-lg font-bold uppercase">
                          {tpl.ten || "Chưa đặt tên mẫu"}
                        </h1>
                        {tpl.mo_ta && (
                          <p className="mt-1 text-xs text-muted-foreground">{tpl.mo_ta}</p>
                        )}
                      </div>
                ...
                <div className="max-h-24 overflow-y-auto border-b bg-amber-50/60 px-4 py-1.5 text-[11px] dark:bg-amber-950/20">
                  <ul className="space-y-0.5">
                    {chkIssues.slice(0, 5).map((iss, k) => (
                      <li
                        key={k}
                        className={iss.level === "error" ? "text-rose-700" : "text-amber-700"}
                      >
                        <AlertTriangle className="mr-1 inline h-3 w-3" />
                        {iss.message}
                      </li>
                    ...
                    {chkIssues.length > 5 && (
                      <li className="text-muted-foreground">… {chkIssues.length - 5} mục nữa</li>
                    )}
                  </ul>
                </div>
              )}
        ...
        <TabsContent value="info" className="h-full mt-0 outline-none overflow-y-auto p-6">
          <div className="mx-auto max-w-3xl space-y-4">
            <Card>
              <CardHeader>
                <CardTitle className="text-base">Thông tin mẫu</CardTitle>
              </CardHeader>
              ...
              <Card>
                <CardHeader>
                  <CardTitle className="text-base">
                    Gắn với hệ thống ({(linkedHt ?? []).length})
                  </CardTitle>
                </CardHeader>
