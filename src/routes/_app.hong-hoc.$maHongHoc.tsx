...
export const Route = createFileRoute("/_app/hong-hoc/$maHongHoc")({
  head: () => ({
    meta: [{ title: "Phiếu hỏng hóc — MIRATS" }, { name: "robots", content: "noindex" }],
  }),
...
function HongHocDetail() {
  ...
  const tbThay = useMemo(
    () =>
      h?.thiet_bi_thay_the
        ? (thietBi.find((t) => t.ma_thiet_bi === h.thiet_bi_thay_the) ?? null)
        : null,
    [thietBi, h],
  );
  const sc = useMemo(
  ...
  const relatedByTb = useMemo(
    () =>
      h
        ? hongHoc
            .filter((x) => x.thiet_bi_hong === h.thiet_bi_hong && x.ma_hong_hoc !== h.ma_hong_hoc)
            .slice(0, 8)
        : [],
    [hongHoc, h],
  );
    ...
    return (
      <div className="mx-auto max-w-lg py-12 text-center">
        ...
        <Button asChild className="mt-4">
          <Link to="/hong-hoc">Quay lại danh sách</Link>
        </Button>
      </div>
    );
  }
  if (!inScope(tb?.don_vi))
  ...
  return (
    <PageFrame density="compact">
      <PageHeader
        ...
        icon={ReplaceIcon}
        breadcrumbs={[{ label: "Hỏng hóc", to: "/hong-hoc" }, { label: h.ma_hong_hoc }]}
        metadata={
        ...
        actions={
          <div className="flex gap-2">
            <Button asChild variant="outline" size="sm" className="h-8">
              <Link to="/hong-hoc">
                <ArrowLeft className="mr-2 h-4 w-4" /> Hỏng hóc
              </Link>
            </Button>
            {sc && (
              <Button asChild variant="outline" size="sm" className="h-8">
                      ...
                      <CardContent className="px-5 pb-5 text-sm space-y-3">
                        ...
                        {h.file_dinh_kem && (
                          <div className="flex items-center gap-2 text-primary pt-2 border-t border-dashed">
                            ...
                            <span className="font-mono text-xs">{h.file_dinh_kem}</span>
                          </div>
                        )}
                      </CardContent>
                    </Card>
                  ...
                  <Card>
                    ...
                    <CardContent className="px-5 pb-5">
                      <div className="grid gap-3 md:grid-cols-[1fr_auto_1fr] md:items-center">
                        ...
                        {tbThay ? (
                          <TbCard
                            title="Tài sản thay thế"
                            tb={tbThay}
                            maFallback={h.thiet_bi_thay_the ?? ""}
                      ...
                      empty={
                        h.vat_tu_su_dung.length === 0 ? (
                          <p className="text-sm text-muted-foreground">
                            Không có vật tư xuất kho cho phiếu này.
                          </p>
                        ) : (
                          <div className="flex flex-wrap gap-2">
                            <div className="text-xs text-muted-foreground w-full mb-1">
                              Dữ liệu cũ (chưa có bút toán xuất kho):
                            </div>
                ...
                <Card>
                  <CardHeader className="pb-3 pt-4 px-5">
                    <CardTitle className="text-sm font-semibold">
                      Lịch sử hỏng hóc khác của tài sản
                    </CardTitle>
                  </CardHeader>
