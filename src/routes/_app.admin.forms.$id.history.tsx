...
export const Route = createFileRoute("/_app/admin/forms/$id/history")({
  head: () => ({
    meta: [
      { title: "Lịch sử phiên bản mẫu — MIRATS" },
                  ...
                  <div className="mt-2 flex flex-wrap gap-1">
                    ...
                    <Button
                      ...
                      onClick={() => {
                        ...
                        if (bundle?.template)
                          downloadBundleJson(
                            bundle,
                            `${q.data?.tpl?.code ?? "template"}-v${r.version}`,
                          );
                    ...
                    <Button
                      ...
                      onClick={() => {
                        if (
                          confirm(
                            `Khôi phục toàn bộ mẫu về v${r.version}? Thao tác này ghi đè fields hiện tại.`,
                          )
                        ) {
                          restoreM.mutate(r);
                        }
        ...
        <main className="min-h-0 overflow-y-auto p-4">
          {!preview ? (
            <p className="mt-16 text-center text-sm text-muted-foreground">
              Chọn 1 phiên bản để xem snapshot.
            </p>
          ) : (
            <Card>
              <CardHeader>
