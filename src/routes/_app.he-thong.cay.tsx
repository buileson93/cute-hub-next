...
import {
  ...
  NONE_HT,
  isRealSystemId,
} from "@/components/mirats/he-thong-cay/utils";
...
export const Route = createFileRoute("/_app/he-thong/cay")({
  ...
  ): { editTb?: string; view?: string; moveHt?: string; moveTb?: string } => ({
  ...
  head: () => ({
    meta: [
      { title: "Cấu trúc & Sơ đồ — MIRATS" },
      {
        name: "description",
        content:
          "Phân lớp hệ thống tài sản: Phân loại → Nhóm hệ thống → Hệ thống → Tài sản → Thành phần.",
      },
...
function HeThongCayPage() {
  ...
  const onRecord = useCallback(
    (kind: "tb" | "tp", ma: string, ten?: string) => {
      if (kind === "tb")
        nav({
  ...
  return (
    <PageFrame density="compact" className="flex flex-col overflow-hidden h-screen">
      <PageHeader
        ...
        subtitle={`BỐI CẢNH... Dự án React + React Flow... SỬA ĐÚNG 5 LỖI NÀY...`}
          ...
          <div className="flex items-center gap-2">
            ...
            <AppTooltip noiDung="Cấu hình sơ đồ">
              <Button
                ...
                className="h-8 w-8 bg-transparent text-primary border-primary/20 hover:bg-primary/5 hover:border-primary/40 shadow-none"
            ...
            {display === "mindmap" && (
              <div className="flex-1 relative">
                <CayMindMap
                  tree={viewTree as any}
                  posByHt={posByHt || new Map()}
                  scopeText={`IMPLEMENTATION MODE — DEAD CONTROLS / RBAC ONLY.
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <DialogDescription className="text-muted-foreground mt-0.5">
                  Thay đổi phân lớp cho hệ thống{" "}
                  <span className="font-semibold text-foreground">
                    {moveSystemTarget?.ten || search.moveHt}
                  </span>
                </DialogDescription>
              </div>
            </div>
                  ...
                  {moveTargets.map((t, idx) => (
                    <button
                      ...
                      onClick={async () => {
                        ...
                        nav({
                          to: "/he-thong/cay",
                          search: (prev: any) => ({ ...prev, moveHt: undefined }),
                        });
                      }}
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <DialogDescription className="text-muted-foreground mt-0.5">
                  Gán tài sản{" "}
                  <span className="font-semibold text-foreground">
                    {moveDeviceTarget?.ten || search.moveTb}
                  </span>{" "}
                  vào hệ thống mới
                </DialogDescription>
              </div>
                  ...
                  {taxonomy?.htList.map((ht) => (
                    <button
                      ...
                      onClick={async () => {
                        ...
                        nav({
                          to: "/he-thong/cay",
                          search: (prev: any) => ({ ...prev, moveTb: undefined }),
                        });
                      }}
