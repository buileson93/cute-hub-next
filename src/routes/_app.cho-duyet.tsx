...
const LOAI_OPTIONS: { value: LoaiFilter; label: string }[] = [
  ...
  ...(Object.entries(LOAI_LABEL) as [ChangeRequestLoai, string][]).map(([value, label]) => ({
    value,
    label,
  })),
];
...
function ChoDuyetPage() {
      ...
      ch = supabase
        ...
        .subscribe((status, err) => {
          if (status === "CHANNEL_ERROR" || status === "TIMED_OUT" || status === "CLOSED") {
            console.warn("[cho-duyet] realtime subscribe status", {
              status,
              channel: channelName,
              error: err?.message,
            });
  ...
  const sortedRows = useMemo(
    () => rows.slice().sort((a, b) => (a.created_at < b.created_at ? 1 : -1)),
    [rows],
  );
      ...
      <div className="p-6">
        <Card className="border-amber-500/30 bg-amber-500/5">
          ...
          <CardContent className="text-sm text-muted-foreground">
            Chỉ vai trò <strong>admin</strong> hoặc <strong>phong_kt</strong> mới xem được hàng đợi
            phê duyệt.
          </CardContent>
        </Card>
      </div>
                ...
                <CardContent className="p-4">
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div className="min-w-0 flex-1 space-y-2">
                      ...
                      {row.ly_do && (
                        <div className="text-sm text-muted-foreground">
                          <span className="font-medium text-foreground">Lý do xử lý:</span>{" "}
                          {row.ly_do}
                        </div>
                      )}
                      {row.error_message && (
                    ...
                    <div className="flex shrink-0 flex-col gap-2">
                      ...
                      {isAdmin && isMine && row.trang_thai === "pending" && (
                        <span className="text-xs text-muted-foreground">
                          Bạn không được tự duyệt CR của mình
                        </span>
                      )}
                    </div>
                  </div>
      ...
      <Dialog open={!!approveTarget} onOpenChange={(open) => !open && setApproveTarget(null)}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              {approveTarget &&
                `${LOAI_LABEL[approveTarget.loai]} — ${summarizePayload(approveTarget.loai, approveTarget.payload)}`}
            </DialogDescription>
          </DialogHeader>
          {approveTarget && !isApplySupported(approveTarget.loai) && (
            <div className="rounded-md border border-amber-500/30 bg-amber-500/5 p-3 text-sm text-amber-700">
              Loại này chưa có dispatcher tự động. Nếu duyệt, hệ thống sẽ đánh dấu{" "}
              <code>applied_failed</code>.
            </div>
          )}
          ...
          <DialogFooter>
            <Button variant="ghost" onClick={() => setApproveTarget(null)} disabled={busy}>
              Đóng
            </Button>
      ...
      <Dialog open={!!rejectTarget} onOpenChange={(open) => !open && setRejectTarget(null)}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              {rejectTarget &&
                `${LOAI_LABEL[rejectTarget.loai]} — ${summarizePayload(rejectTarget.loai, rejectTarget.payload)}`}
            </DialogDescription>
          </DialogHeader>
          <div className="space-y-2">
