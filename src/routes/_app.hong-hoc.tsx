  ...
  head: () => ({
    meta: [
      { title: "Hỏng hóc & Thay thế — MIRATS" },
      {
        name: "description",
        content:
          "M6 — Phiếu hỏng hóc–thay thế, truy vết tài sản cũ → mới, xuất kho vật tư sửa chữa.",
      },
      { property: "og:title", content: "Hỏng hóc & Thay thế — MIRATS" },
      {
        property: "og:description",
        content: "Số hoá phiếu hỏng hóc và liên kết chuỗi truy vết linh kiện.",
      },
    ],
  }),
    ...
    const base: StdColumn<HongHocThayThe>[] = [
      {
        ...
        cell: (h) => (
          <Link
            ...
            params={{ maHongHoc: h.ma_hong_hoc }}
      ...
      },
      {
        ...
        value: (h) => h.ngay_hong,
        cell: (h) => (
          <span className="whitespace-nowrap text-xs text-muted-foreground">{h.ngay_hong}</span>
        ),
      },
      {
        key: "thiet_bi_hong",
      ...
      },
      {
        ...
        value: (h) => h.phuong_an,
        cell: (h) => (
          <Badge variant="secondary" className={getPhuongAnHongHocToken(h.phuong_an)?.class}>
            {h.phuong_an}
          </Badge>
        ),
      },
      {
        ...
        hideBelow: "2xl",
        value: (h) =>
          h.thiet_bi_thay_the ? (thietBiMap.get(h.thiet_bi_thay_the)?.ma_thiet_bi ?? "") : "",
        cell: (h) => {
          const tbT = h.thiet_bi_thay_the ? thietBiMap.get(h.thiet_bi_thay_the) : null;
          return tbT ? (
            <Link
              to="/thiet-bi/$maThietBi"
              params={{ maThietBi: tbT.ma_thiet_bi }}
              search={{ tab: "tong-quan", doc: undefined, q: undefined }}
              className="flex items-center gap-1 text-primary hover:underline"
            >
              <ArrowRight className="h-3 w-3" />
              <span className="font-mono text-xs">{tbT.ma_thiet_bi}</span>
            </Link>
          ) : (
    ...
    <PageFrame density="compact">
      ...
      <PageBody className="space-y-4">
        <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
          ...
          <Kpi
            icon={DollarSign}
            label="Chi phí sửa/thay"
            value={`${fmtVND(stats.chi_phi)} đ`}
                  ...
                  <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
                    ...
                    <Combobox
                      ...
                      options={[
                        ...
                        ...donVi
                          .filter((d) => d.ma !== "CTY")
                          .map((d) => ({ value: d.ma, label: `${d.ma} — ${d.ten}` })),
                      ]}
                    />
                    <Select value={pa} onValueChange={(v) => setPa(v)}>
                      <SelectTrigger>
                        <SelectValue placeholder="Phương án" />
                      </SelectTrigger>
                      <SelectContent>
                        <SelectItem value="all">Mọi phương án</SelectItem>
                        {["Sửa chữa", "Thay thế", "Thanh lý"].map((s) => (
  ...
  return (
    ...
    >
      <DialogContent className="max-w-lg">
        <DialogHeader>
          <DialogTitle>Sửa phiếu {row?.ma_hong_hoc}</DialogTitle>
        </DialogHeader>
        <div className="space-y-3">
          ...
          {paNorm === "thay_the" && (
            <div>
              ...
              <Input
                value={thayTheId}
                onChange={(e) => setThayTheId(e.target.value)}
                placeholder="Nhập mã tài sản hoặc UUID"
              />
