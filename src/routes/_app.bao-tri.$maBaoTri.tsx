...
import { 
  ...
  LayoutList, ClipboardCheck, History, Info as InfoIcon, Settings, ArrowRight, Building2, ReplaceIcon,
} from "lucide-react";
...
function BaoTriDetail() {
  ...
  return (
    <PageFrame density="compact">
      ...
      <PageBody>
        <VongDoiPanel bang={"bao_tri" as any} id={(bt as any).ma_bao_tri} trangThaiHienTai={bt.trang_thai} />
        <div className="mb-6" />
          ...
          tabs={[
                    ...
                    <Card>
                      ...
                      <CardContent className="px-5 pb-5 text-sm space-y-3">
                        ...
                        <div>
                          ...
                          {tb ? (
                            <Link to="/thiet-bi/$maThietBi" params={{ maThietBi: tb.ma_thiet_bi }} search={{ tab: "tong-quan", doc: undefined, q: undefined }} className="text-primary hover:underline font-medium">
                    ...
                    <Card className="lg:col-span-2">
                      ...
                      <CardContent className="px-5 pb-5 text-sm space-y-4">
                        ...
                        <div className="flex flex-wrap items-center justify-between gap-3 pt-2 border-t border-dashed">
                          <div className="flex items-center gap-2">
                            ...
                            <div className="flex -space-x-2">
                              {bt.nguoi_thuc_hien.map((n: string) => (
                                <div key={n} title={n} className="h-7 w-7 rounded-full border-2 border-background bg-secondary grid place-items-center text-[10px] font-bold uppercase ring-1 ring-border">
                                  {n.slice(0, 2)}
                                </div>
                              ))}
                            </div>
                          ...
                          {bt.file_bien_ban && (
                            <Button variant="ghost" size="sm" className="h-7 text-primary gap-1 px-2">
                              <FileText className="h-3.5 w-3.5" />
                              <span className="font-mono text-xs underline">{bt.file_bien_ban}</span>
                            </Button>
                          )}
                        </div>
                      </CardContent>
                  ...
                  <CardHeader className="pb-3 pt-4 px-5 flex flex-row items-center justify-between space-y-0">
                    ...
                    <div className="flex gap-2">
                      {Object.entries(kqCount).map(([k, v]) => (
                        <Badge key={k} variant="secondary" className={cn("text-[10px] py-0", kqColor[k])}>{k}: {v}</Badge>
                      ))}
                    </div>
                  </CardHeader>
                          ...
                          {hangMuc.length === 0 ? (
                            ...
                            return (
                              <TableRow key={h.id}>
                                ...
                                <TableCell className="font-medium text-sm">{h.ten_hang_muc}</TableCell>
                                <TableCell className="text-sm tabular-nums font-mono">{h.gia_tri_do || "—"}</TableCell>
                                <TableCell className="text-xs text-muted-foreground">{h.tieu_chuan || "—"}</TableCell>
            ...
            },
            {
              ...
              content: (
                <Card>
                  <CardHeader className="pb-3 pt-4 px-5">
                    <CardTitle className="text-sm font-semibold">Lịch sử bảo dưỡng gần đây của tài sản</CardTitle>
                  </CardHeader>
                  <CardContent className="p-0 border-t">
                    <Table>
                      <TableHeader>
                      ...
                      <TableBody>
                        {other.length === 0 ? (
                          <TableRow>
                            <TableCell colSpan={4} className="h-24 text-center text-muted-foreground italic">Không có bản ghi khác.</TableCell>
                          </TableRow>
                        ) : other.map((x) => (
