...
function SuCoDetail() {
  ...
  useEffect(() => {
    setVdPick(sc?.van_de_id ?? "");
  }, [sc?.van_de_id]);

  const linkedVd = useMemo(
    () => vdOpts.find((o) => o.value === sc?.van_de_id),
    [vdOpts, sc?.van_de_id],
  );
  ...
  async function saveVanDe(newId: string | null) {
    ...
    const { error } = await supabase
      .from("su_co")
      .update({ van_de_id: newId })
      .eq("ma_su_co", sc.ma_su_co);
    setSaving(false);
      ...
      <PageHeader
        ...
        icon={() => (
          <AlertTriangle
            className={cn(
              "h-4 w-4",
              sc.muc_do === "Nghiêm trọng" ? "text-destructive" : "text-primary",
            )}
          />
        )}
        breadcrumbs={[{ label: "Sự cố", to: "/su-co" }, { label: sc.ma_su_co }]}
        metadata={
        ...
        actions={
          <div className="flex gap-2">
            <Button asChild variant="outline" size="sm" className="h-8">
              <Link to="/su-co">
                <ArrowLeft className="mr-2 h-4 w-4" /> Nhật ký
              </Link>
            </Button>
      ...
      <PageBody>
        <div className="grid grid-cols-2 gap-3 md:grid-cols-4 mb-6">
          <KpiCard
            icon={Clock}
            label="Downtime"
            value={fmtDowntime(sc.thoi_gian_gian_doan)}
            tone={sc.thoi_gian_gian_doan ? "text-amber-600" : "text-muted-foreground"}
          />
          ...
          tabs={[
            {
              ...
              content: (
                <div className="space-y-6">
                  {sc.muc_do === "Nghiêm trọng" && (
                    <div className="flex items-start gap-3 rounded-md border border-red-200 bg-red-50 p-4 text-sm text-red-800">
                      ...
                      <div>
                        ...
                        <div>
                          Đã gửi cảnh báo cho phụ trách đơn vị {dv?.ten ?? sc.don_vi} và Phòng Kỹ
                          thuật.
                        </div>
                      </div>
                    </div>
                  ...
                  {suCoId && (
                    <VongDoiPanel bang="su_co" id={suCoId} trangThaiHienTai={sc.trang_thai} />
                  )}
                      ...
                      <CardContent className="px-5 pb-5">
                        <div className="space-y-3 text-sm">
                          <InfoRow
                            ...
                            value={sc.ngay_phat_hien.replace("T", " ")}
                          />
                          <InfoRow
                            icon={Calendar}
                            label="Thời điểm khắc phục"
                            value={
                              sc.thoi_diem_khac_phuc
                                ? sc.thoi_diem_khac_phuc.replace("T", " ")
                                : "Chưa khắc phục"
                            }
                          />
                          <InfoRow
                      ...
                      <CardContent className="px-5 pb-5">
                        <div className="space-y-4 text-sm">
                          ...
                          <Section
                            ...
                            title="Nguyên nhân"
                            tone="text-amber-600"
                            body={sc.nguyen_nhan ?? "Chưa xác định."}
                          />
                          <Section
                            icon={Wrench}
                            title="Biện pháp xử lý"
                            tone="text-emerald-600"
                            body={sc.bien_phap_xu_ly ?? "Đang cập nhật."}
                          />
                        </div>
                      </CardContent>
                    </Card>
                    ...
                    <VatTuTieuHaoView
                      ...
                      empty={
                        <p className="text-sm text-muted-foreground">
                          Chưa có bút toán xuất kho cho sự cố này.
                        </p>
                      }
                    />
                    {coQuyenGhi && suCoId && (
                      ...
                      {!sc.lien_ket_hong_hoc ? (
                        <Button asChild variant="outline" size="sm">
                          <Link
                            ...
                            search={{
                              suCo: sc.ma_su_co,
                              heThong: sc.he_thong_id ?? undefined,
                              thietBi: sc.thiet_bi,
                            }}
                          >
                        ...
                        <Button asChild variant="outline" size="sm">
                          <Link
                            ...
                            params={{ maHongHoc: sc.lien_ket_hong_hoc }}
                          >
                  ...
                  <CardContent className="space-y-4 pt-6 text-sm">
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      <span className="text-muted-foreground">
                        {linkedVd?.label ?? "Chưa gắn vào vấn đề (RCA) nào"}
                      </span>
                      <Button asChild variant="outline" size="sm">
                    ...
                    >
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-xs text-muted-foreground">
                          {x.ngay_phat_hien.replace("T", " ")}
                        </span>
                      </div>
                      <div className="min-w-0 flex-1 truncate px-2 text-muted-foreground">
                        {x.hien_tuong}
                      </div>
                      <div className="flex items-center gap-2">
                        <StatusBadge domain="su_co" code={x.muc_do} label={x.muc_do} />
                        <span className="text-xs tabular-nums text-muted-foreground">
                          {fmtDowntime(x.thoi_gian_gian_doan)}
                        </span>
                      </div>
                    </Link>
                  ))}
