...
function checkFile(filePath: string) {
  ...
  if (
    ...
    relativePath.includes("/integrations/supabase/")
  )
    return;
  ...
  if (!relativePath.includes("AstryxProvider.tsx")) {
    if (
      content.match(
        /^(const|let|var|export const|export let).+=\s*(window|document|requestAnimationFrame|localStorage)/m,
      )
    ) {
      violations.push({
        file: relativePath,
  ...
  if (
    ...
    !ALLOWED_ISLANDS.some((island) => relativePath.includes(island))
  ) {
    violations.push({
      file: relativePath,
