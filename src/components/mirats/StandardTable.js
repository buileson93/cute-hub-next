...
import { DropdownMenu, DropdownMenuCheckboxItem, DropdownMenuContent, DropdownMenuLabel, DropdownMenuSeparator, DropdownMenuTrigger, DropdownMenuItem, DropdownMenuSub, DropdownMenuSubTrigger, DropdownMenuPortal, DropdownMenuSubContent, } from "@/components/ui/dropdown-menu";
...
export function StandardTableInner({ className, rows = [], columns = [], getRowId, selectable, selected: selectedProp, setSelected: setSelectedProp, selection, setSelection, maxHeightClass = "h-full", emptyText = "Không có dữ liệu", emptyContent, errorContent, trangThai = {}, loadingContent, onRowClick, rowClassName, toolbarRight, toolbarLeft, bulkActions, bulkActionsActions, tableKey, countUnit = "bản ghi", requireFilterToShow, pagination, clientPagination, presets, activePreset, hideReorderToggle, editMode, onColumnsChange, virtualizerOptions, infiniteScroll, expandable, renderExpansion, }) {
    ...
    useEffect(() => {
        if (typeof window === "undefined" || !parentRef.current || !window.ResizeObserver)
            return;
        let frameId;
    ...
    const toggleExpand = useCallback((rid) => {
    ...
    const viewMode = useMemo(() => {
        if (containerWidth === 0)
            return "desktop";
        if (containerWidth < UI_DENSITY.CONT_SM)
            return "mobile";
        if (containerWidth < UI_DENSITY.CONT_MD)
            return "tablet";
        return "desktop";
    }, [containerWidth]);
    ...
    const shownCols = useMemo(() => {
        return sortedColumns.filter((c) => {
            if (tableKey && prefs.hidden.has(c.key))
                return false;
            if (c.hidden)
                return false;
            // Priority-based hiding
            if (viewMode === "tablet" && c.priority === "detail")
                return false;
            if (!c.hideBelow)
                return true;
            const threshold = typeof c.hideBelow === "number" ? c.hideBelow : BP_PX[c.hideBelow] || BP_PX.md;
            // Responsive hideBelow không làm mất cột sticky hoặc action
            if (c.sticky || c.key === "actions")
                return true;
    ...
    const matchesFilters = useCallback((r, exceptKey) => {
        for (const c of columns) {
            ...
            if (c.filter === "cat") {
                ...
                if (sel && sel.size > 0 && !sel.has(colText(c, r)))
                    return false;
            }
                ...
                if (val) {
                    ...
                    if (t && !normalize(colText(c, r)).includes(t))
                        return false;
                }
            }
        }
    ...
    const sortableKey = useCallback((c) => c.sortable ?? !!(c.sortValue || c.value), []);
    const cycleSort = useCallback((key) => setSort((prev) => {
    ...
    const display = useMemo(() => {
        ...
        if (infiniteScroll)
            return rows;
        if (!clientPagination || virtualizerOptions?.enabled === true)
            return fullDisplay;
        const { page, pageSize } = clientPagination;
        if (pageSize >= fullDisplay.length)
            return fullDisplay;
    ...
    useEffect(() => {
        if (!infiniteScroll || !infiniteScroll.hasNextPage || infiniteScroll.isFetchingNextPage)
            return;
    ...
    const paddingBottom = displayItems.length > 0 ? totalSize - (displayItems[displayItems.length - 1]?.end || 0) : 0;
    // --- Kéo đổi độ rộng cột ---
    ...
    const renderGlobalState = () => {
        if (trangThai.loi) {
            return (errorContent ?? (<div className="py-20 flex flex-col items-center justify-center text-center gap-4 border rounded-lg bg-card">
            <div className="text-sm text-destructive font-medium">{String(trangThai.loi)}</div>
            {trangThai.loi.retry && (<Button variant="outline" size="sm" onClick={trangThai.loi.retry}>
                Thử lại
              </Button>)}
          </div>));
        }
        if (trangThai.dangTai) {
            return (<div className="p-4 border rounded-lg bg-card">
          {loadingContent ?? (<TableSkeleton cols={isMobile ? 1 : shownCols.length} rows={isMobile ? 3 : 6}/>)}
        </div>);
        }
        if (fullDisplay.length === 0) {
            return (<div className="py-20 border rounded-lg bg-card">
          <EmptyState title={hasFilter
                    ? "Không có dòng nào khớp bộ lọc"
                    : gated
                        ? "Vui lòng chọn bộ lọc để xem dữ liệu"
                        : emptyContent
                            ? undefined
                            : emptyText} description={hasFilter
                    ? "Vui lòng thử điều chỉnh hoặc xoá các bộ lọc đang bật"
                    : typeof emptyContent === "string"
                        ? emptyContent
                        : undefined} action={hasFilter ? (<Button variant="outline" size="sm" onClick={clearAllFilters} className="mt-4">
                  Xoá tất cả bộ lọc
                </Button>) : undefined} live="polite"/>
    ...
    function renderAutoCell(c, r) {
        ...
        switch (c.type) {
            ...
            case "number":
                return (<span className="tabular-nums font-mono text-right w-full inline-block pr-1 truncate">
            {fmtSo(Number(val))}
          </span>);
            case "currency":
                return (<span className="tabular-nums font-mono text-right w-full inline-block pr-1 truncate">
            ...
            {val ? (<Check className="h-4 w-4 text-emerald-500"/>) : (<XIcon className="h-4 w-4 text-muted-foreground/30"/>)}
          </div>);
            case "path":
            ...
            case "longtext":
                return (<AppTooltip noiDung={String(val)}>
            <div className={cn("text-[12px] break-words [overflow-wrap:anywhere] [word-break:break-word]", (c.lineClamp ?? 1) > 1 ? `line-clamp-${c.lineClamp}` : "truncate")}>
              {String(val)}
            </div>
          </AppTooltip>);
            case "actions":
            ...
            default:
                ...
                return (<div className={cn("text-[12px] break-words [overflow-wrap:anywhere] [word-break:break-word]", (c.lineClamp ?? 1) > 1 ? `line-clamp-${c.lineClamp}` : "truncate")} title={String(val)}>
            {String(val)}
          </div>);
        }
    }
    ...
    return (<div className="space-y-1">
      ...
      {(toolbarRight || toolbarLeft || (selectable && selectedRows.length > 0)) && (<div className="flex items-center justify-between gap-1 px-0">
          ...
          <div className="flex items-center gap-1">
            {tableKey && (<>
                {isMobile ? (<DropdownMenu>
                    ...
                    <DropdownMenuContent align="end" className="w-[280px]">
                      <div className="p-2 space-y-4">
                        ...
                        <div className="space-y-3">
                          {columns
                        .filter((c) => c.filter)
                        .map((c) => (<ColFilter key={c.key} type={c.filter || "text"} label={c.header || c.label || ""} catValues={catValues[c.key] || []} catSel={catFilters[c.key] || new Set()} onToggleCat={(val) => toggleCat(c.key, val)} onClearCat={() => clearCat(c.key)} textVal={textFilters[c.key] || ""} onText={(val) => setTextFilters((prev) => ({ ...prev, [c.key]: val }))}/>))}
                        </div>
                      </div>
                    </DropdownMenuContent>
                    ...
                    <TooltipProvider>
                      <Tooltip>
                        <TooltipTrigger asChild>
                          <DropdownMenuTrigger asChild>
                            <Button variant="outline" size="default" className="h-auto w-auto p-1.5 ml-1 border-primary/20 hover:bg-primary/5 hover:border-primary/40 shadow-none" disabled={!prefs.ready}>
                    ...
                    <DropdownMenuContent align="end" className="w-56">
                          ...
                          <DropdownMenuSub>
                            ...
                            <DropdownMenuPortal>
                              <DropdownMenuSubContent className="w-48">
                                {presets.map((p) => (<DropdownMenuItem key={p.id} className="text-xs flex flex-col items-start gap-0.5" onClick={() => {
                                ...
                                prefs.setPreset(p.id, finalKeys, p.orderKeys || finalKeys);
                                    ...
                                    {p.moTa && (<span className="text-[10px] text-muted-foreground leading-tight">
                                        {p.moTa}
                                      </span>)}
                                  </DropdownMenuItem>))}
                      ...
                      <div className="flex flex-col gap-1 p-1">
                        <Button variant="ghost" size="sm" className="h-7 px-2 justify-start text-xs font-normal" onClick={() => {
                      ...
                      {Object.entries(sortedColumns.reduce((acc, col) => {
                        const g = col.group || "Khác";
                        if (!acc[g])
                            acc[g] = [];
                        acc[g].push(col);
                        return acc;
                    }, {})).map(([group, cols], idx, arr) => (<div key={group}>
                          {group !== "Khác" && (<DropdownMenuLabel className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground pt-2">
                              {group}
                            </DropdownMenuLabel>)}
                          {cols.map((col) => {
                            ...
                            return (<DropdownMenuCheckboxItem key={col.key} checked={isCurrentlyVisible} onCheckedChange={() => prefs.toggle(col.key)} onSelect={(e) => e.preventDefault()} disabled={!canToggle} className="flex items-center justify-between gap-2">
                                <span className={cn(isHiddenByWidth &&
                                    isCurrentlyVisible &&
                                    "text-muted-foreground/60", col.inherited &&
                                ...
                                {isHiddenByWidth && isCurrentlyVisible && (<AppTooltip noiDung={`Cột này đang tạm ẩn do màn hình hẹp (< ${thresholdLabel}). Hãy mở rộng trình duyệt hoặc xem ở dòng mở rộng.`}>
                      ...
                      <DropdownMenuSeparator />
                      <DropdownMenuCheckboxItem className="text-primary focus:text-primary font-medium" onSelect={(e) => {
                        e.preventDefault();
                        prefs.reset();
                    }}>
                        Đặt lại mặc định
                ...
                <div className="flex items-center">
                  {!hideReorderToggle && (<AppTooltip noiDung="Bật/tắt chế độ kéo thả đổi thứ tự cột">
                      <Button variant="ghost" size="icon" className={cn("h-7 w-7 ml-0.5 transition-colors", internalReorder && "text-primary bg-primary/10")} onClick={() => setInternalReorder(!internalReorder)}>
      ...
      {hasFilter && (<div className="flex flex-wrap items-center gap-2 px-1">
          ...
          {Object.entries(textFilters).map(([key, val]) => {
                ...
                return (<Badge key={key} variant="secondary" size="sm" className="gap-1 px-2 py-0.5 h-6 font-medium bg-secondary/50">
                <span className="text-muted-foreground">{col?.header || col?.label}:</span>
                <span className="truncate max-w-[120px]">{val}</span>
                <button onClick={() => setTextFilters((prev) => {
                        const { [key]: _, ...rest } = prev;
                        return rest;
          ...
          {Object.entries(catFilters).map(([key, sel]) => {
                ...
                <button onClick={() => clearCat(key)} className="hover:text-destructive transition-colors">
                  <Icon name="action.close" size="tiny"/>
                </button>
              </Badge>);
            })}
          <Button variant="ghost" size="sm" onClick={clearAllFilters} className="h-7 px-3 text-[11px] font-bold uppercase tracking-wide text-muted-foreground/60 hover:text-destructive gap-2">
            <Icon name="action.close" size="tiny"/>
      ...
      {isMobile ? (<div className="space-y-3">
        ...
        </div>) : (<div className={cn("relative min-h-0 border rounded-md shadow-none bg-background astryx-table-container flex flex-col h-full", className)}>
          <div ref={parentRef} className="flex-1 overflow-auto mirats-scroll w-full">
            <Table className={cn("caption-bottom min-w-full astryx-table", density === "compact" && "text-[12px]", density === "comfortable" && "text-[13px]", density === "spacious" && "text-[14px]", prefs.layoutMode === "auto" ? "w-max table-auto" : "w-full table-fixed")}>
            ...
            <TableHeader className="bg-muted/30 sticky top-0 z-30 shadow-sm shadow-border/50">
              <TableRow className={cn("hover:bg-transparent border-b border-border/60 align-middle", UI_DENSITY.TABLE_ROW_H, "astryx-table-header")}>


                {viewMode === "tablet" && (<TableHead className="sticky left-0 top-0 z-30 w-10 bg-muted/30 border-r border-border/50 p-0">
                    {/* Placeholder for expansion column header */}
                  </TableHead>)}
                {selectable && (<TableHead scope="col" className={cn("sticky left-0 top-0 z-30 w-10 bg-muted/30 border-r border-border/50 p-0", viewMode === "tablet" && "left-10")}>
                    <div className="flex h-full w-full items-center justify-center">
                      <Checkbox checked={display.length > 0 && selected?.size === display.length
                    ? true
                    : (selected?.size ?? 0) > 0
                        ? "indeterminate"
                        : false} onCheckedChange={(checked) => {
                    if (checked)
                        setSelected?.(new Set(display.map(getRowIdInternal)));
                    else
                        clearSelection();
                ...
                return (<TableHead key={c.key} scope="col" aria-sort={sortActive ? (sort.dir === "asc" ? "ascending" : "descending") : "none"} className={cn("group relative astryx-table-header-cell p-0", c.sticky && "sticky left-0 z-30", selectable && c.sticky && "left-10", c.align === "center" && "text-center", c.align === "right" && "text-right", sortActive && "bg-primary/5")} style={{
                        height: "100%",
                    }}>
                      <div className={cn("flex items-center gap-1 h-full w-full", c.align === "right" && "justify-end", c.align === "center" && "justify-center")}>
                        {reorder && (<div className="h-6 w-4 flex items-center justify-center cursor-grab active:cursor-grabbing text-muted-foreground/30 hover:text-muted-foreground/60 transition-colors" draggable onDragStart={(e) => {
                        ...
                        {canSort ? (<button type="button" onClick={() => cycleSort(c.key)} className={cn("group inline-flex min-w-0 items-center gap-1.5 rounded-md hover:bg-accent/50 hover:text-foreground transition-all focus-visible:ring-1 focus-visible:ring-primary focus-visible:outline-none h-full px-2 py-1", c.align === "center"
                            ? "justify-center w-full"
                            : c.align === "right"
                                ? "justify-end w-full text-right"
                                : "justify-start text-left w-full")} title={`Sắp xếp theo ${label}`} aria-label={`Sắp xếp theo ${label}`}>
                            <span className="truncate">{label}</span>
                            {sortActive ? (sort.dir === "asc" ? (<Icon name="table.sortAsc" size="tiny" className="text-primary"/>) : (<Icon name="table.sortDesc" size="tiny" className="text-primary"/>)) : (<Icon name="table.sortNone" size="tiny" className="text-muted-foreground/30 group-hover:text-muted-foreground/60"/>)}
                          </button>) : (<span className="truncate">{c.header || c.label || ""}</span>)}

                        {c.filter && (<ColFilter type={c.filter} label={c.header || c.label || ""} catValues={catValues[c.key] ?? []} catSel={catFilters[c.key] ?? new Set()} onToggleCat={(v) => toggleCat(c.key, v)} onClearCat={() => clearCat(c.key)} textVal={textFilters[c.key] ?? ""} onText={(v) => setTextFilters((p) => ({ ...p, [c.key]: v }))}/>)}
                      </div>

                      {/* Resizer handle */}
                      <div role="separator" tabIndex={0} aria-label={`Thay đổi độ rộng cột ${label}`} className="absolute right-0 top-0 h-full w-1.5 cursor-col-resize opacity-0 group-hover:opacity-100 hover:bg-primary/30 transition-opacity z-10 focus-visible:opacity-100 focus-visible:bg-primary/30 outline-none" onMouseDown={(e) => onHandleMouseDown(e, c.key, currentWidth)} onDoubleClick={() => prefs.resetWidth(c.key)} onKeyDown={(e) => {
            ...
            <TableBody>
              {renderGlobalState() ? (<TableRow>
                  <TableCell colSpan={shownCols.length + (selectable ? 1 : 0) + (viewMode === "tablet" ? 1 : 0)} className="p-0 border-0">
                    {renderGlobalState()}
                  </TableCell>
                </TableRow>) : (<>
                  {paddingTop > 0 && (<TableRow className="hover:bg-transparent">
                      <TableCell colSpan={shownCols.length + (selectable ? 1 : 0) + (viewMode === "tablet" ? 1 : 0)} style={{ height: `${paddingTop}px` }} className="p-0 border-0 pointer-events-none bg-transparent"/>
                    </TableRow>)}
                  ...
                  {displayItems.map((virtualRow) => {
                    ...
                    /** Tự động render ô dựa trên `type` */
                    return (<React.Fragment key={`row-${rid}-${virtualRow.key}`}>
                        <TableRow key={virtualRow.key} data-index={virtualRow.index} ref={(el) => {
                            rowVirtualizer.measureElement(el);
                            // Nếu có lineClamp hoặc nội dung phức tạp, TanStack Virtual sẽ tự đo lại qua ref này
                        }} className={cn("group astryx-table-row", (onRowClick || selectable) && "cursor-pointer", isSel && "selected", expandedRows.has(rid) && "expanded", UI_DENSITY.TABLE_ROW_H, rowClassName?.(r))} onClick={() => onRowClick?.(r)} onKeyDown={(e) => {
                          ...
                          {viewMode === "tablet" && (<TableCell onClick={(e) => {
                                e.stopPropagation();
                                toggleExpand(rid);
                          ...
                          {selectable && (<TableCell onClick={(e) => e.stopPropagation()} className={cn("astryx-table-cell sticky left-0 z-10 bg-inherit p-0", viewMode === "tablet" && "left-10")}>
                              <div className="flex h-full w-full items-center justify-center">
                          ...
                          {shownCols.map((c, colIdx) => {
                            const savedW = prefs.widths[c.key];
                            return (<TableCell key={c.key} scope={colIdx === 0 ? "row" : undefined} className={cn(c.cellClassName, "astryx-table-cell", (c.type === "number" ||
                                    c.type === "currency" ||
                                    c.type === "percent") &&
                                    "astryx-table-cell-numeric", c.sticky &&
                                    "sticky left-0 z-10 bg-inherit", selectable && c.sticky && "left-10", c.align === "center" && "text-center", c.align === "right" && "text-right tabular-nums", c.inherited && "bg-amber-50/50 dark:bg-amber-950/20")} style={{
                                    width: savedW
                                        ? `${savedW}px`
                                        : c.minW
                                            ? c.minW.includes("[")
                        ...
                        {expandedRows.has(rid) && (<TableRow className="bg-muted/10 border-b border-border/20">
                            <TableCell colSpan={shownCols.length +
                                (selectable ? 1 : 0) +
                                (viewMode === "tablet" ? 1 : 0)} className="p-4">
                              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 animate-in fade-in slide-in-from-top-1 duration-200">
                                ...
                                .map((col) => (<div key={col.key} className="flex flex-col gap-1 min-w-0 text-left">
                  ...
                  {paddingBottom > 0 && (<TableRow className="hover:bg-transparent">
                  ...
                  {infiniteScroll?.isFetchingNextPage && (<TableRow className="hover:bg-transparent">
                      <TableCell colSpan={shownCols.length + (selectable ? 1 : 0) + (viewMode === "tablet" ? 1 : 0)} className="p-4 text-center border-0 bg-muted/5 animate-pulse">
                        <div className="flex items-center justify-center gap-2 text-xs text-muted-foreground">
                          <Loader2 className="h-3 w-3 animate-spin"/> Đang tải thêm dữ liệu...
                        </div>
                      </TableCell>
                    </TableRow>)}
                  {!infiniteScroll?.hasNextPage && infiniteScroll && display.length > 0 && (<TableRow className="hover:bg-transparent">
                      <TableCell colSpan={shownCols.length + (selectable ? 1 : 0) + (viewMode === "tablet" ? 1 : 0)} className="py-6 text-center border-0 opacity-40 italic text-[11px] text-muted-foreground">
...
function ColFilter({ type, label, catValues, catSel, onToggleCat, onClearCat, textVal, onText, }) {
    const [open, setOpen] = useState(false);
    const [q, setQ] = useState("");
    const active = type === "text" ? textVal.length > 0 : catSel.size > 0;
    return (<DropdownMenu open={open} onOpenChange={setOpen}>
      <DropdownMenuTrigger asChild>
        <Button variant="ghost" size="icon" className={cn("h-6 w-6 ml-auto shrink-0 transition-colors", active
            ? "text-[#0074e2] bg-[#0074e2]/10"
            : "text-muted-foreground/30 hover:text-[#0074e2] hover:bg-[#0074e2]/5")} onClick={(e) => {
            e.stopPropagation();
        }}>
      ...
      <DropdownMenuContent align="start" className="w-64 p-2 shadow-xl border-border/50" onClick={(e) => e.stopPropagation()} side="bottom">
        <div className="flex items-center justify-between mb-2 px-1">
          ...
          {active && (<Button variant="ghost" size="sm" className="h-5 px-1.5 text-[10px] text-destructive hover:bg-destructive/10" onClick={() => {
                if (type === "text")
                    onText("");
                else
                    onClearCat();
        ...
        {type === "text" ? (<div className="space-y-2">
            <div className="relative">
              ...
              <Input autoFocus placeholder="Tìm nội dung..." value={textVal} onChange={(e) => onText(e.target.value)} className="h-8 pl-8 text-sm"/>
            </div>
            ...
            <div className="relative">
              <Icon name="action.search" size="small" className="absolute left-2 top-2.5 text-muted-foreground"/>
              <Input autoFocus placeholder="Tìm giá trị..." value={q} onChange={(e) => setQ(e.target.value)} className="h-8 pl-8 text-sm"/>
            </div>
            <div className="max-h-[300px] overflow-y-auto pr-1">
              {catValues
                .filter((v) => !q || normalize(v.value).includes(normalize(q)))
                .map((v) => (<DropdownMenuCheckboxItem key={v.value} checked={catSel.has(v.value)} onCheckedChange={() => onToggleCat(v.value)} onSelect={(e) => e.preventDefault()} className="text-sm py-1.5 px-2 cursor-pointer flex items-center justify-between">
              ...
              {catValues.length === 0 && (<div className="py-6 text-center text-xs text-muted-foreground italic">
...
export function StandardTable(props) {
    ...
    const getRowIdInternal = useCallback((row) => {
        if (props.getRowId)
    ...
    return (<div className="flex flex-col h-full relative">
      ...
      {(props.selectable || props.selected || props.selection) &&
            ...
            props.bulkActionsActions && (<BulkActionBar selectedCount={selectedRows.length} onClear={clearSelection} actions={props.bulkActionsActions.map((a) => ({
                label: a.label,
                icon: a.icon,
                variant: a.variant,
