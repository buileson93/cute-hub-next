...
export function StandardTable({ rows = [], columns = [], getRowId, selectable, selected, setSelected, maxHeightClass = "max-h-[600px]", emptyText = "Không có dữ liệu", emptyContent, errorContent, trangThai, loadingContent, onRowClick, rowClassName, toolbarRight, toolbarLeft, bulkActions, tableKey, countUnit = "bản ghi", requireFilterToShow, pagination, clientPagination, presets, activePreset, }) {
    ...
    const getRowIdInternal = (r) => (getRowId ? getRowId(r) : r.id);
    const toggleRow = (id) => {
        if (!setSelected)
            return;
        setSelected((prev) => {
    ...
    const hasFilter = useMemo(() => {
        return columns.some((c) => c.filter === "cat" ? (catFilters[c.key]?.size ?? 0) > 0
    ...
    return (<div className="space-y-3">
      {(toolbarRight || toolbarLeft || (selectable && selectedRows.length > 0)) && (<div className="flex items-center justify-between gap-2 px-1">
          <div className="flex items-center gap-2">
            {toolbarLeft && renderToolbar(toolbarLeft, { visibleRows: fullDisplay, visibleColumns: shownCols })}
            {selectable && selectedRows.length > 0 && bulkActions && (bulkActions({
                selectedRows,
                      ...
                      <Tooltip>
                        <TooltipTrigger asChild>
                          <DropdownMenuTrigger asChild>
                            <Button variant="outline" size="sm" className="h-8 gap-2 ml-1" disabled={!prefs.ready}>
                      ...
                      {Object.entries(sortedColumns.reduce((acc, col) => {
                        ...
                        return acc;
                          ...
                          {cols.map((col) => {
                            ...
                            return (<DropdownMenuCheckboxItem key={col.key} checked={isCurrentlyVisible} onCheckedChange={() => prefs.toggle(col.key)} onSelect={(e) => e.preventDefault()} disabled={!canToggle}>
                                {col.label}
      ...
      {hasFilter && (<div className="flex flex-wrap items-center gap-2 px-1">
          ...
          {Object.entries(textFilters).map(([key, val]) => {
                ...
                return (<Badge key={key} variant="secondary" className="gap-1 px-2 py-0.5 h-6">
          ...
          <Button variant="ghost" size="sm" onClick={clearAllFilters} className="h-6 px-2 text-[10px] font-bold uppercase tracking-wider text-muted-foreground/60 hover:text-destructive">
            Xoá tất cả bộ lọc
          </Button>
        </div>)}

      {isMobile ? (<div className="space-y-3">
          {fullDisplay.length === 0 ? (<div className="py-20 text-center text-sm text-muted-foreground border rounded-lg bg-card">
              {hasFilter ? "Không có dòng nào khớp bộ lọc" : (gated ? "Bảng đang trống." : (emptyContent ?? emptyText))}
                ...
                return (<Card key={rid} className={cn("relative cursor-pointer transition-colors hover:bg-muted/50", isSel && "border-primary bg-primary/5", rowClassName?.(r))} onClick={() => onRowClick?.(r)}>
                  <CardContent className="p-4">
                    {selectable && (<div className="absolute right-3 top-3">
                        <Checkbox checked={isSel} onCheckedChange={() => toggleRow(rid)} onClick={(e) => e.stopPropagation()}/>
                      </div>)}
                    <div className="space-y-3">
                      {shownCols.slice(0, 5).map((col) => (<div key={col.key} className="flex flex-col gap-0.5">
        ...
        </div>) : (<Card ref={parentRef} className={cn("relative min-h-0 overflow-auto border shadow-sm", maxHeightClass)}>
          <Table className="w-full table-fixed border-separate border-spacing-0 caption-bottom text-sm">
            <TableHeader className="bg-muted sticky top-0 z-20 shadow-[0_1px_0_hsl(var(--border))]">
              <TableRow className="hover:bg-transparent">
                {selectable && (<TableHead className="sticky left-0 top-0 z-30 w-10 bg-muted border-r border-border/50"></TableHead>)}
                {shownCols.map((c) => {
                ...
                return (<TableHead key={c.key} className={cn("group relative bg-muted border-r border-border/50 last:border-r-0", c.sticky && "sticky left-0 z-30", selectable && c.sticky && "left-10", c.align === "center" && "text-center", c.align === "right" && "text-right", sortActive && "bg-primary/5")} style={{
                      ...
                      <div className={cn("flex items-center gap-1", c.align === "right" && "justify-end", c.align === "center" && "justify-center")}>
                        {canSort ? (<button type="button" onClick={() => cycleSort(c.key)} className="group inline-flex min-w-0 items-center gap-1 rounded hover:text-foreground text-left" title="Bấm để sắp xếp">
                            <span className="truncate">{c.label}</span>
                            {sortActive ? (sort.dir === "asc"
                            ? <ArrowUp className="h-3 w-3 shrink-0 text-primary"/>
                            : <ArrowDown className="h-3 w-3 shrink-0 text-primary"/>) : (<ChevronsUpDown className="h-3 w-3 shrink-0 text-muted-foreground/30 group-hover:text-muted-foreground/60"/>)}
                          </button>) : (<span className="truncate">{c.label}</span>)}

                        {c.filter && (<ColFilter type={c.filter} label={c.label} catValues={catValues[c.key] ?? []} catSel={catFilters[c.key] ?? new Set()} onToggleCat={(v) => toggleCat(c.key, v)} onClearCat={() => clearCat(c.key)} textVal={textFilters[c.key] ?? ""} onText={(v) => setTextFilters((p) => ({ ...p, [c.key]: v }))}/>)}
                      </div>

                      {/* Resizer handle */}
                      <div className="absolute right-0 top-0 h-full w-1.5 cursor-col-resize opacity-0 group-hover:opacity-100 hover:bg-primary/30 transition-opacity z-10" onMouseDown={(e) => onHandleMouseDown(e, c.key, currentWidth)} onDoubleClick={() => prefs.resetWidth(c.key)} title="Kéo để đổi độ rộng — bấm đúp để đặt lại"/>
                    </TableHead>);
            })}
              </TableRow>
            </TableHeader>
            <TableBody>
              {trangThai?.loi ? (<TableRow>
                  <TableCell colSpan={shownCols.length + (selectable ? 1 : 0)} className="h-24">
                    {errorContent ?? (<div className="flex h-full items-center justify-center text-sm text-destructive">
                        {String(trangThai.loi)}
                      </div>)}
                  </TableCell>
                </TableRow>) : trangThai?.dangTai ? (<TableRow>
                  <TableCell colSpan={shownCols.length + (selectable ? 1 : 0)} className="h-24">
                    {loadingContent ?? <TableSkeleton cols={shownCols.length}/>}
                  </TableCell>
                </TableRow>) : rows.length === 0 ? (<TableRow>
                  <TableCell colSpan={shownCols.length + (selectable ? 1 : 0)} className="h-24">
                    <EmptyState title={hasFilter ? "Không có dòng nào khớp bộ lọc" : (emptyContent ? undefined : emptyText)} description={hasFilter ? "Vui lòng thử điều chỉnh hoặc xoá các bộ lọc đang bật" : (typeof emptyContent === "string" ? emptyContent : undefined)} action={hasFilter ? (<Button variant="outline" size="sm" onClick={clearAllFilters} className="mt-4">
                          Xoá tất cả bộ lọc
                        </Button>) : undefined}/>
                  ...
                  {virtualRows.map((virtualRow) => {
                    ...
                    const rid = getRowIdInternal(r);
                    const isSel = selectable && selected?.has(rid);
                    return (<TableRow key={rid} data-index={virtualRow.index} ref={rowVirtualizer.measureElement} className={cn(onRowClick && "cursor-pointer", isSel && "bg-primary/5", rowClassName?.(r))} onClick={() => onRowClick?.(r)}>
                        {selectable && (<TableCell onClick={(e) => e.stopPropagation()} className="sticky left-0 z-10 bg-card border-r border-border/50">
                            <Checkbox checked={isSel} onCheckedChange={() => toggleRow(rid)}/>
                          </TableCell>)}
                        {shownCols.map((c) => {
                            const savedW = prefs.widths[c.key];
                            return (<TableCell key={c.key} className={cn(c.cellClassName, c.sticky && "sticky left-0 z-10 bg-card border-r border-border/50", selectable && c.sticky && "left-10", c.align === "center" && "text-center", c.align === "right" && "text-right tabular-nums", c.inherited && "bg-amber-50/50 dark:bg-amber-950/20")} style={{
                                    width: savedW ? `${savedW}px` : (c.minW ? (c.minW.includes('[') ? c.minW.match(/\[(.*?)\]/)?.[1] : c.minW) : undefined),
                                    minWidth: savedW ? `${savedW}px` : (c.minW ? (c.minW.includes('[') ? c.minW.match(/\[(.*?)\]/)?.[1] : c.minW) : undefined)
                                }}>
                            {c.cell ? (c.cell(r)) : (<div className={cn("break-words [overflow-wrap:anywhere] [word-break:break-word]", (c.lineClamp ?? 1) > 1
                                        ? `line-clamp-${c.lineClamp}`
                                        : "truncate")} title={String(c.value?.(r) ?? "")}>
                                {String(c.value?.(r) ?? "")}
                              </div>)}
                          </TableCell>);
                        })}
...
function ColFilter({ type, label, catValues, catSel, onToggleCat, onClearCat, textVal, onText, }) {
    ...
    return (<DropdownMenu open={open} onOpenChange={setOpen}>
      <DropdownMenuTrigger asChild>
        <Button variant="ghost" size="icon" className={cn("h-6 w-6 ml-auto shrink-0 transition-colors", active ? "text-primary bg-primary/10" : "text-muted-foreground/30 hover:text-muted-foreground hover:bg-muted")} onClick={(e) => {
            e.stopPropagation();
        }}>
