      ...
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          ...
          {opts.noi_dung_chi_tiet && (
            <p className="mt-0.5 whitespace-pre-line text-meta text-foreground/80">
              {opts.noi_dung_chi_tiet}
            </p>
          )}
          {item.huong_dan && (
        ...
        <div className="flex shrink-0 flex-col items-end gap-1">
          {thresholdLabel && (
            <Badge variant="secondary" className="font-mono text-mini" title="Ngưỡng tự chấm">
              {thresholdLabel}
            </Badge>
          )}
          {item.tieu_chuan && (
            <span className="rounded bg-secondary px-2 py-0.5 text-mini text-muted-foreground">
              TC: {item.tieu_chuan}
            </span>
          )}
...
function LiveStatusBadge({
  ...
  if (!hasValue) {
    return (
      <Badge variant="outline" className="gap-1 text-mini">
  ...
  if (status === "dat") {
    return (
      <Badge className="gap-1 border-emerald-500/40 bg-emerald-500/15 text-mini text-emerald-700 hover:bg-emerald-500/15 dark:text-emerald-300">
  ...
  if (status === "khong_dat") {
    return (
      <Badge className="gap-1 border-rose-500/40 bg-rose-500/15 text-mini text-rose-700 hover:bg-rose-500/15 dark:text-rose-300">
...
function KetQuaBadge({ value }: { value: KetQua }) {
  ...
  return (
    <Badge className={cn("gap-1 text-mini hover:bg-transparent", cls)}>
