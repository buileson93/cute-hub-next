...
function useThietBiOptions(search: string = "") {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      if (search) {
        query = query.or(
          `ten_thiet_bi.ilike.%${search}%,ma_thiet_bi.ilike.%${search}%,nhan_vien.ho_ten.ilike.%${search}%`,
        );
      }
      ...
      return (data ?? []).map((r: any) => ({
        ...
        subLabel: r.nhan_vien
          ? `${r.nhan_vien.ho_ten} (${r.nhan_vien.don_vi || "KĐĐ"})`
          : "Chưa gán người sử dụng",
  ...
  const capPhatMut = useMutation({
    mutationFn: async () => {
      ...
      if (!selectedTb?.nhanVien) {
        throw new Error(
          "Tài sản chưa được gán nhân viên phụ trách. Vui lòng cập nhật danh mục trước khi cấp phát.",
        );
      }
      if (!selectedTb.hasModel || !selectedTb.hasSerial) {
        throw new Error(
          "Tài sản thiếu thông tin Model hoặc Số Serial. Vui lòng bổ sung thông tin này.",
        );
      }
      ...
      if (data) {
        await import("@/lib/mirats/ban-quyen-detail").then((m) =>
          m.logBanQuyenAudit(
            data.ban_quyen_id,
            "REVOKE",
            `Thu hồi bản quyền từ thiết bị ID: ${data.thiet_bi_id}`,
          ),
        );
      }
    },
      ...
      <DialogContent className="max-w-3xl">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            Cấp phát:{" "}
            {activeBq
              ? (activeBq as any).ten_phan_mem || (activeBq as any).label
              : "Chọn bản quyền"}
          </DialogTitle>
          <DialogDescription>
            {activeBq
          ...
          <div className="flex flex-col gap-4 rounded-xl border-2 border-primary/10 bg-primary/5 p-4 transition-all hover:border-primary/20">
            {!banQuyen && (
              <div className="space-y-1.5">
                <label className="text-xs font-semibold uppercase tracking-wider text-primary/70 ml-1">
                  Bản quyền phần mềm
                </label>
                <Combobox
                  className="w-full"
          ...
          {isLoading ? (
            ...
            <ul className="divide-y">
              {capPhat.map((c: any) => (
                <li
                  key={c.id}
                  className="group flex items-center justify-between gap-3 p-4 transition-colors hover:bg-primary/5"
                >
