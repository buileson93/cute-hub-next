...
export function ThietBiLifecycleActions({
  ma,
  trangThai,
}: {
  ma: string;
  ...
  const retire = useMutation({
    ...
    onSuccess: (d) => {
      ...
      setLyDo("");
      toast.success(
        `Đã chuyển sang "${d?.trang_thai ?? "Ngừng khai thác"}" — hồ sơ lý lịch được giữ nguyên`,
      );
    },
  ...
  const restore = useMutation({
    mutationFn: async () => {
      const { error } = await supabase.rpc("phuc_hoi_thiet_bi", {
        _mas: [ma],
        _ly_do: lyDo || undefined,
      });
      if (error) throw error;
    ...
    onSuccess: () => {
      invalidate();
      setLyDo("");
      toast.success("Đã phục hồi tài sản về Đang khai thác");
    },
    onError: (e: unknown) => toast.error(e instanceof Error ? e.message : "Không thực hiện được"),
  });
        ...
        <AlertDialog>
          ...
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                Tài sản <b className="font-mono">{ma}</b> sẽ chuyển sang trạng thái ngừng khai thác.
                Toàn bộ lịch sử (sự cố, bảo dưỡng, hỏng hóc, bàn giao, kiểm kê){" "}
                <b>được giữ nguyên</b> và vẫn tra cứu được.
              </AlertDialogDescription>
            </AlertDialogHeader>
            <div className="space-y-3">
              <div className="flex items-center gap-2 rounded-md border bg-muted/20 px-3 py-2">
                <div className="flex-1 space-y-0.5">
                  <Label htmlFor="thanh-ly-check" className="text-sm font-medium cursor-pointer">
                    Thanh lý / loại biên
                  </Label>
                  <p className="text-xs text-muted-foreground">
                    Tài sản sẽ không còn trong danh mục sẵn dùng.
                  </p>
                </div>
              ...
              <div className="space-y-1">
                ...
                <Textarea
                  ...
                  placeholder="Ví dụ: hết niên hạn, hư hỏng không sửa được…"
                  rows={2}
                />
              </div>
            </div>
        ...
        <AlertDialog>
          ...
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                Chỉ dùng cho <b>bản ghi nhập nhầm</b>. Thao tác chỉ thành công nếu tài sản{" "}
                <b>chưa phát sinh</b> sự cố, bảo dưỡng, hỏng hóc, bàn giao, kiểm kê, giấy phép hay
                phiếu nào — nếu đã có lịch sử, hệ thống sẽ từ chối và bạn nên dùng “Ngừng khai
                thác”. Không thể hoàn tác.
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
              <AlertDialogCancel>Huỷ</AlertDialogCancel>
