...
import {
  ...
  LOAI_META,
  TRANG_THAI_META,
  type CongVanLienKetLoai,
  type CongVanLinkRow,
  type CongVanLoai,
  type CongVanRow,
  type CongVanTepRow,
  type CongVanTrangThai,
  extractCanCuSo,
  fmtDate,
} from "./types";
...
export function CongVanSheet({
  ...
  const goiYCanCu = useMemo(() => {
    ...
    const found = allCongVan.find(
      (c) =>
        c.id !== editing?.id &&
        c.so_cong_van.replace(/\s+/g, "").toLowerCase().startsWith(so.toLowerCase()),
    );
    return found ?? null;
  }, [draft.can_cu_text, draft.trich_yeu, allCongVan, editing]);

  async function save() {
    if (!draft.so_cong_van.trim()) {
      toast.error("Nhập số công văn");
      return;
    }
    setSaving(true);
    try {
  ...
  async function addLink() {
    ...
    if (error) toast.error(error.message);
      ...
      toast.success("Đã nối công văn kế nhiệm");
      setLinkTarget("");
      onDone();
    }
  }
  ...
  async function upload(file: File) {
    if (!editing) {
      toast.error("Lưu công văn trước khi đính kèm");
      return;
    }
  ...
  async function openTep(t: CongVanTepRow) {
    ...
    setViewer({
      id: t.id,
      url: (data as { signedUrl?: string } | null)?.signedUrl ?? null,
      name: t.file_name,
      mime: t.mime_type,
    });
  }
      ...
      <Sheet open={open} onOpenChange={onOpenChange}>
        ...
        >
          <SheetHeader className="pb-2">
            ...
            <SheetDescription>
              {editing
                ? "Chi tiết & liên kết theo dòng thời gian"
                : "Thêm công văn vào dòng thời gian dự án"}
            </SheetDescription>
          </SheetHeader>
          ...
          {editing && (
            <>
              ...
              <section className="space-y-2">
                ...
                <div className="space-y-1 text-xs">
                  {preds.map((l) => (
                    <LinkRow
                      key={l.id}
                      text={`← ${byId.get(l.tu_id)?.so_cong_van ?? "?"} · ${LIEN_KET_META[l.loai].label}`}
                      canEdit={canEdit}
                  ...
                  {succs.map((l) => (
                    <LinkRow
                      key={l.id}
                      text={`→ ${byId.get(l.den_id)?.so_cong_van ?? "?"} · ${LIEN_KET_META[l.loai].label}`}
                      canEdit={canEdit}
                      onRemove={() => void removeLink(l.id)}
                    />
                  ))}
                ...
                {canEdit && (
                  <div className="flex flex-wrap items-center gap-2 pt-1">
                    ...
                    <Select
                      value={linkLoai}
                      onValueChange={(v) => setLinkLoai(v as CongVanLienKetLoai)}
                    >
              ...
              <section className="space-y-2 pb-6">
                ...
                {!myTeps.length && (
                  <p className="text-xs text-muted-foreground">Chưa có bản scan.</p>
                )}
                {canEdit && (
                  <label
                    className={cn(
