...
const MUC_BY_PL: Record<string, string> = {
  A: "Nghiêm trọng",
  B: "Cao",
  C: "Trung bình",
  D: "Thấp",
...
export function SuCoMoiForm({
  ...
  const selected = useMemo(
    () =>
      Array.from(new Set(mounted.map((m) => m.device.id))).map(
        (id) => mounted.find((m) => m.device.id === id)!.device,
      ),
    [mounted],
  );
  ...
  const anomalies = useMemo(
    () =>
      detectSuCoAnomalies({
        ...
        mountedAssetsCount: selected.length,
      }),
    ...
    () =>
      createVoiceRecognition({
        onTranscript: (text, isFinal) => {
          setAiText(text);
          if (isFinal) parseMutation.mutate(text);
        },
        onEnd: () => setVoiceActive(false),
        onError: (m) => {
          setVoiceActive(false);
          toast.error("Lỗi voice: " + m);
        },
      }),
    [parseMutation],
  );
  ...
  useEffect(() => {
    ...
    if (d?.transcript) {
      setAiText(d.transcript);
      parseMutation.mutate(d.transcript);
    }
    if (defaultVoice) {
      setAiText(defaultVoice);
      parseMutation.mutate(defaultVoice);
    }
    ...
    supabase
      ...
      .then(({ data }) => {
        setVanDeOptions(
          (data ?? []).map((v) => ({
            value: v.id!,
            label: v.tieu_de ?? "(không tiêu đề)",
            hint: v.ma_van_de ?? undefined,
          })),
        );
      });
  }, []);
  ...
  useEffect(() => {
    if (tpList.length === 0 || selectedTpIds.size === 0) {
      setMounted([]);
      return;
    }
    ...
    supabase
      .from("gan_chuc_nang")
      .select("thanh_phan_id, thiet_bi(*)")
      .in("thanh_phan_id", tps)
      .is("den_ngay", null)
      .then(({ data }) => {
        const list: MountedAsset[] = (data ?? [])
          .filter((d) => d.thiet_bi)
          .map((d) => ({
            thanh_phan_id: d.thanh_phan_id!,
            device: d.thiet_bi as unknown as DbDevice,
          }));
        setMounted(list);
      });
  ...
  function validateBeforeSave(closing: boolean): string | null {
    ...
    if (selected.length === 0)
      return "Các thành phần đã chọn chưa lắp tài sản nào — không thể ghi sự cố";
    if (closing) {
  ...
  const buildPayloadForSave = (maNhom: string, closing: boolean) =>
    buildSuCoPayload({
      ma_nhom_bc: maNhom,
      ...
      bien_phap_xu_ly: bienPhapXuLy || null,
      bao_cao_ban_dau: {
        kinh_gui: kinhGui,
        ...
        dia_diem: "",
        kip_truc: kip,
        thanh_phan_list: tpList.filter((t) => selectedTpIds.has(t.id)),
      ...
      devices: selected.map((d) => ({
        ...
        he_thong_id: (d as any)._htId ?? null,
        he_thong_ten: (d as any)._htTen ?? null,
      })),
    });
  ...
  const save = useMutation({
    mutationFn: async () => {
      ...
      if (closingIntent && res?.ids?.length && thoiGianKetThuc) {
        await supabase
          .from("su_co")
          .update({
            thoi_diem_khac_phuc: new Date(thoiGianKetThuc).toISOString(),
            at_hoan_thanh: new Date(thoiGianKetThuc).toISOString(),
    ...
    onSuccess: (maNhom) => {
      ...
      if (maNhom) {
        exportFn({ data: { ma_nhom_bc: maNhom } })
          .then((res) => {
            if (res?.base64) {
    ...
    <div className={embedded ? "flex h-full flex-col" : "mx-auto max-w-5xl space-y-3 pb-28"}>
      ...
      <div className="flex-1 overflow-y-auto px-4">
        {step === 1 && (
          <div className="space-y-4">
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-1.5">
                  ...
                  <Input
                    type="date"
                    value={ngayPhatHien}
                    onChange={(e) => setNgayPhatHien(e.target.value)}
                  />
                </div>
              </div>
            </CollapsibleSection>
              ...
              action={
                <div className="flex gap-2">
                  <Button
                    ...
                    className={voiceActive ? "bg-red-50 text-red-600" : ""}
                  >
            ...
            >
              <div className="space-y-4">
                <div className="space-y-1.5">
                  ...
                  <Textarea
                    value={hienTuong}
                    onChange={(e) => setHienTuong(e.target.value)}
                    placeholder="Mô tả hiện tượng..."
                        ...
                        {PHAN_LOAI.map((l) => (
                          <SelectItem key={l} value={l}>
                            <div className="flex items-center gap-2">
                              <Badge
                                variant={l === "A" || l === "B" ? "destructive" : "outline"}
                ...
                <div className="space-y-1.5">
                  ...
                  <AssetPicker
                    value={heThongDichVu}
                    onChange={(id) => setHeThongDichVu(id)}
                    heThongId={heThongId}
                  />
                </div>
              </div>
            ...
            >
              <VisionImageHint
                onApplyDescription={(text: string) =>
                  setHienTuong((prev) => (prev ? prev + "\n" + text : text))
                }
                onApplyCategory={(cat: "A" | "B" | "C" | "D" | "E") => setPhanLoai(cat)}
                onApplyKeywords={(keywords: string[]) =>
                  setTomTat((prev) =>
                      ...
                      <li key={i} className="flex gap-2 text-[12px] leading-relaxed">
                        ...
                        <div className="flex flex-col">
                          <span
                            className={cn(
                              "font-medium",
                              a.severity === "error" ? "text-red-700" : "text-amber-800",
                            )}
                          >
                            {a.message}
                          </span>
          ...
          <Card>
            <CardHeader>
              <CardTitle className="text-sm font-semibold text-primary">
                2. Chọn thành phần hệ thống & Tài sản
              </CardTitle>
            </CardHeader>
              ...
              <div className="space-y-2 max-h-[40vh] overflow-y-auto pr-2">
                {tpLoading ? (
                  ...
                  <p className="text-center text-xs text-muted-foreground py-8">
                    Hệ thống chưa có thành phần nào.
                  </p>
                ) : (
                  tpList.map((tp) => (
                    <div
                    ...
                    >
                      <Checkbox
                        ...
                        onCheckedChange={(v) => {
                          const n = new Set(selectedTpIds);
                          v ? n.add(tp.id) : n.delete(tp.id);
                          setSelectedTpIds(n);
                        }}
                      />
                      <Label htmlFor={`tp-${tp.id}`} className="flex-1 cursor-pointer">
                        {tp.ten}
                      </Label>
                    </div>
                  ))
              ...
              <CardContent className="space-y-4">
                <div className="space-y-1.5">
                  ...
                  <Textarea
                    ...
                    placeholder="Tóm tắt ngắn gọn diễn biến..."
                  />
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1.5">
                    ...
                    <Input
                      type="datetime-local"
                      value={thoiGianBatDau.slice(0, 16)}
                  ...
                  <div className="space-y-1.5">
                    ...
                    <Input
                      type="datetime-local"
                      value={thoiGianKetThuc.slice(0, 16)}
                ...
                <div className="pt-4 space-y-4">
                  <div className="flex items-center justify-between rounded-lg border border-primary/20 bg-primary/5 p-3">
                    <div className="space-y-0.5">
                      <Label
                        htmlFor="closing-intent"
                        className="text-sm font-semibold text-primary"
                      >
                        Chốt đóng sự cố ngay
                      </Label>
                      <p className="text-xs text-muted-foreground">
                        Tự động điền thời gian kết thúc và chuyển trạng thái hoàn thành.
                      </p>
                    </div>
                    <Switch
                      id="closing-intent"
                      checked={closingIntent}
                      onCheckedChange={(v: boolean) => setClosingIntent(!!v)}
                    />
                  </div>
                    ...
                    <div className="space-y-4 animate-in fade-in slide-in-from-top-2 duration-300">
                      <div className="space-y-1.5">
                        ...
                        <Textarea
                          ...
                          placeholder="Trạng thái hệ thống/tài sản sau khi xử lý..."
                        />
                      </div>
                      <div className="space-y-1.5">
                        ...
                        <Textarea
                          ...
                          placeholder="Kết luận nguyên nhân..."
                        />
                      </div>
                      <div className="space-y-1.5">
                        ...
                        <Textarea
                          ...
                          placeholder="Các bước đã thực hiện để khắc phục..."
                        />
                      </div>
                      <div className="space-y-1.5">
                        ...
                        <Textarea
                          ...
                          placeholder="Đánh giá hiệu quả sau xử lý..."
                        />
                      </div>
                    </div>
                  )}
      ...
      <div className="sticky bottom-0 flex items-center justify-between border-t p-4 bg-background">
        ...
        <div className="flex gap-2">
          ...
          >
            <FileDown className="h-4 w-4 mr-1" /> Xem trước
          </Button>
          {step < 3 ? (
            <Button
              onClick={() => {
                ...
                if (step === 1) {
                  if (!hienTuong.trim()) {
                    toast.error("Vui lòng nhập hiện tượng");
                    return;
                  }
                  if (!heThongId) {
                    toast.error("Vui lòng chọn hệ thống");
                    return;
                  }
                  if (!heThongDichVu) {
            ...
            >
              {save.isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : "Ghi sự cố"}
            </Button>
          )}
        </div>
      </div>
