...
export function HongHocMoiForm({
  defaultSuCo,
  defaultHeThongId,
  defaultThietBi,
  embedded,
  ...
  const { data: htList } = useQuery({
    ...
    queryFn: async () => (await supabase.from("dm_he_thong").select("id, ma, ten")).data ?? [],
  });
  const { data: tbAll } = useQuery({
    queryKey: ["thiet_bi_min_for_hh"],
    queryFn: async () =>
      (await supabase.from("thiet_bi").select("id, ma_thiet_bi, ten_thiet_bi")).data ?? [],
  });
  ...
  useEffect(() => {
    if (currentDevice?.thiet_bi_id) setThietBiHongId(currentDevice.thiet_bi_id);
  }, [currentDevice]);
  ...
  const validate = () => {
    ...
    if (step === 2) {
      if (!boPhan) {
        toast.error("Vui lòng nhập bộ phận hỏng");
        return false;
      }
      if (!thietBiHongId) {
        toast.error("Vui lòng chọn tài sản hỏng");
        return false;
      }
      if (phuongAn === "thay_the" && !thietBiThayTheId) {
        toast.error("Vui lòng chọn tài sản thay thế");
        return false;
      }
    }
    if (step === 3) {
      if (!moTa.trim()) {
        toast.error("Vui lòng nhập mô tả hỏng hóc");
        return false;
      }
    }
    return true;
  ...
  const steps = [
    { id: 1, title: "Nguồn gốc" },
    { id: 2, title: "Tài sản & Phương án" },
    { id: 3, title: "Mô tả chi tiết" },
  ];
    ...
    <div className={embedded ? "flex h-full flex-col" : "mx-auto max-w-5xl space-y-3 pb-28"}>
      ...
      <div className="flex-1 overflow-y-auto px-4">
        {step === 1 && (
          <Card>
            <CardHeader>
              <CardTitle className="text-sm font-semibold text-primary">1. Nguồn gốc</CardTitle>
            </CardHeader>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <div className="space-y-1.5">
                ...
                <Combobox
                  options={(tpList ?? []).map((t) => ({ value: t.id, label: t.ten }))}
                  value={thanhPhanId}
                  onChange={setThanhPhanId}
                />
              </div>
              ...
              <div className="space-y-1.5">
                ...
                <Input
                  ...
                  placeholder="VD: Khối nguồn, Màn hình, Cáp tín hiệu..."
                />
              </div>
