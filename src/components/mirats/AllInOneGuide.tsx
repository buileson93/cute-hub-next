...
import { BookOpen, KeyRound, Link2, AlertTriangle, CheckCircle2, Bot, Layers } from "lucide-react";
        ...
        <div className="mt-4 space-y-4 text-sm">
          <section className="space-y-1.5">
            ...
            <ul className="space-y-1 text-xs text-muted-foreground">
              <li className="flex gap-1.5">
                ...
                <span>
                  <b className="text-foreground">ma</b> = khoá upsert. Giữ ma → cập nhật, trống ma →
                  tạo mới.
                </span>
              </li>
              <li className="flex gap-1.5">
                <Link2 className="mt-0.5 h-3 w-3 shrink-0 text-primary" />
                <span>Cha trước, con sau. Điền sheet theo thứ tự 1 → 10.</span>
              </li>
              <li className="flex gap-1.5">
                <CheckCircle2 className="mt-0.5 h-3 w-3 shrink-0 text-primary" />
                <span>Idempotent — nhập lại không nhân bản.</span>
              </li>
            </ul>
          </section>
          ...
          <section className="space-y-1.5">
            <h3 className="flex items-center gap-1.5 font-semibold">
              <AlertTriangle className="h-3.5 w-3.5 text-amber-500" /> Lỗi hay gặp
            </h3>
            <ul className="space-y-1 text-xs text-muted-foreground">
              <li>• Trỏ danh mục chưa tồn tại → khai ở sheet cha trước.</li>
              <li>
                • Đổi <code>ma</code> cũ → tạo bản trùng.
              </li>
              <li>
                • Trùng <code>ma_serial</code> → để trống nếu chưa có.
              </li>
          ...
          <section className="space-y-1.5 rounded-md border border-dashed border-primary/40 bg-primary/5 p-2.5">
            <h3 className="flex items-center gap-1.5 text-xs font-semibold">
              <Bot className="h-3.5 w-3.5 text-primary" /> Dành cho AI agent
            </h3>
            <ol className="list-decimal space-y-0.5 pl-4 text-[11px] text-muted-foreground">
              <li>
                Xuất kèm dữ liệu để lấy đúng <code>ma</code> hợp lệ.
              </li>
              <li>
                Điền theo thứ tự 1 → 10, cột <code>ref</code> chỉ dùng giá trị đã tồn tại.
              </li>
              <li>
                Chỉ điền <code>model</code> ở dòng tài sản — không lặp NSX/Loại.
              </li>
