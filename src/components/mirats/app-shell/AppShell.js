...
import { workspaces, resolveActiveWorkspace, firstItemOf, } from "@/lib/mirats/nav-contract";
...
function TourAutoStart({ userId, seen, onSeen, }) {
    const { start } = useProductTour();
    const ran = useRef(false);
    useEffect(() => {
        if (!userId || seen || ran.current)
            return;
        ran.current = true;
        const t = setTimeout(() => {
            start({ force: true });
...
export function AppShell({ children }) {
    ...
    useRouteTracker();
    const [wsLastRoute, setWsLastRoute] = useState({});
    const [flyoutWs, setFlyoutWs] = useState(null);
    ...
    useEffect(() => {
        try {
            ...
            if (raw)
                setWsLastRoute(JSON.parse(raw));
        }
        catch { /* ignore */ }
    }, []);
    const visibleWorkspaces = useMemo(() => workspaces.filter((ws) => !ws.roles || ws.roles.some((r) => hasRole(r))), [hasRole]);
    const railWorkspaces = useMemo(() => visibleWorkspaces.filter((w) => w.id !== "trao-doi" && w.id !== "he-thong"), [visibleWorkspaces]);
    const adminWs = useMemo(() => visibleWorkspaces.find((w) => w.id === "he-thong") ?? null, [visibleWorkspaces]);
    ...
    return (<ProductTourProvider steps={TOUR_STEPS}>
      <TourAutoStart userId={!loading && session && profile ? profile.id : null} seen={profile?.tour_hoan_thanh ?? true} onSeen={refresh}/>
        ...
        <div className="flex min-h-dvh w-full bg-gradient-to-br from-background via-background to-primary/[0.045] text-foreground">
          ...
          <aside className="hidden w-16 shrink-0 flex-col items-center border-r border-sidebar-border bg-gradient-to-b from-sidebar via-sidebar to-sidebar/92 py-4 md:flex">
            ...
            <nav data-tour="rail" className="flex flex-1 flex-col items-center gap-1.5">
              {railWorkspaces.map((ws) => (<button key={ws.id} onClick={() => gotoWorkspace(ws)} className={cn("group relative flex w-[54px] flex-col items-center gap-1 rounded-xl px-1 py-1.5 text-[9.5px] font-medium transition-colors", ws.id === activeWs.id ? "bg-accent text-primary" : "text-muted-foreground hover:bg-secondary hover:text-foreground")}>
                  <ws.icon className="h-5 w-5"/>
                  <span className="w-full truncate text-center leading-tight">{ws.short}</span>
                  {ws.id === activeWs.id && (<motion.div layoutId="active-ws" className="absolute -right-[1px] top-1/2 h-6 w-[3px] -translate-y-1/2 rounded-l-full bg-primary"/>)}
                </button>))}
            </nav>
            {adminWs && (<div className="mt-auto pt-2 border-t border-sidebar-border">
                <button onClick={() => gotoWorkspace(adminWs)} className={cn("flex w-[54px] flex-col items-center gap-1 rounded-xl px-1 py-1.5 text-[9.5px] font-medium transition-colors", adminWs.id === activeWs.id ? "bg-accent text-primary" : "text-muted-foreground hover:bg-secondary hover:text-foreground")}>
          ...
          {/* Sub-sidebar (Desktop) */}
          <aside className={cn("hidden shrink-0 flex-col border-r border-border bg-sidebar/30 md:flex transition-[width] duration-200", isCollapsed ? "w-[3.25rem]" : "w-64")}>
            <div className={cn("flex h-14 items-center border-b px-6 font-bold tracking-tight overflow-hidden whitespace-nowrap", isCollapsed && "px-0 justify-center opacity-0 w-0")}>
              {activeWs.label}
            </div>
            ...
            <div className="mt-auto border-t p-2 flex justify-center">
              <Button variant="ghost" size="icon" className="h-8 w-8 text-muted-foreground hover:text-foreground" onClick={toggleCollapsed} aria-label={isCollapsed ? "Mở rộng thanh điều hướng" : "Thu gọn thanh điều hướng"}>
          ...
          <div className="flex min-w-0 flex-1 flex-col">
            <header className="sticky top-0 z-10 flex h-14 items-center justify-between gap-3 border-b border-border bg-background/90 px-4 backdrop-blur-md md:px-6">
               <div className="flex flex-1 items-center gap-4">
                  ...
                  <TopBar renderMobileMenu={<Sheet open={isMobileMenuOpen} onOpenChange={setIsMobileMenuOpen}>
