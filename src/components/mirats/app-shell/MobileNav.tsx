...
import { type Workspace, workspaces, firstItemOf } from "@/lib/mirats/nav-contract";
...
export function MobileNav({ activeWsId, wsLastRoute }: MobileNavProps) {
  ...
  const navWorkspaces = visibleWorkspaces.filter((w) => w.id !== "trao-doi" && w.id !== "he-thong");

  function gotoWorkspace(ws: Workspace) {
    const remembered = wsLastRoute[ws.id];
    const target = remembered && remembered.startsWith("/") ? remembered : firstItemOf(ws, hasRole);
    ...
    <nav className="fixed inset-x-0 bottom-0 z-40 flex items-stretch gap-1 border-t border-border bg-background/95 px-2 pb-[max(0.5rem,env(safe-area-inset-bottom))] pt-1 backdrop-blur-md md:hidden shadow-[0_-4px_12px_rgba(0,0,0,0.05)]">
      {navWorkspaces.map((ws) => {
        ...
        return (
          ...
          >
            <ws.icon
              className={cn("h-5 w-5", isActive ? "text-[#0074e2]" : "text-muted-foreground")}
            />
