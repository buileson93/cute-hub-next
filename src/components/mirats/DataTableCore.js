 ...
 */
export function DataTableCore({ rows, columns, getRowId, selectable, selected, onSelect, onRowClick, className, maxHeight: initialMaxHeight, fitViewport = false, virtualize = false, onLoadMore, hasMore = false, }) {
    const [density] = useDensity();
    const containerRef = useRef(null);
    const [calculatedMaxHeight, setCalculatedMaxHeight] = useState(initialMaxHeight);
    ...
    useEffect(() => {
        if (!onLoadMore || !hasMore || !virtualize)
            return;
    ...
    useLayoutEffect(() => {
        if (!fitViewport || !containerRef.current)
            return;
        const calculate = () => {
    ...
    const tableClasses = cn("relative w-full astryx-table", density === "compact" ? "text-[12px]" : "text-[13px]");
    return (<div ref={containerRef} className={cn("relative overflow-auto mirats-scroll rounded-md bg-card mirats-data-table-core h-full flex-1 min-h-0", className)} style={{
            overflowX: 'auto',
            overflowY: 'auto',
            position: 'relative',
      ...
      <Table className={cn(tableClasses, "mirats-data-table-core-element whitespace-nowrap min-w-full flex flex-col")} style={{
            tableLayout: 'fixed',
            width: 'max-content',
            minWidth: '100%'
        }}>
        <TableHeader className="sticky top-0 z-50 bg-muted/95 backdrop-blur-[4px] flex flex-col shadow-sm">
          <TableRow className="bg-transparent border-b-0 border-t-0 astryx-table-row hover:bg-transparent flex items-stretch">
            {selectable && (<TableHead style={{ width: 40, minWidth: 40, flex: '0 0 40px' }} className="w-10 px-3 text-center sticky left-0 z-50 bg-inherit astryx-table-header-cell"/>)}
            {columns.map((col) => (<TableHead key={col.key} style={{
                width: col.width,
                ...
                background: 'inherit'
            }} className={cn("astryx-table-header-cell", col.sticky && "border-r border-border/20", col.align === "center" && "text-center", col.align === "right" && "text-right")}>
                {col.header}
              </TableHead>))}
          </TableRow>
        </TableHeader>
        <TableBody className="flex flex-col" style={{
          ...
          {rows.length === 0 ? (<TableRow className="border-0">
            ...
            </TableRow>) : virtualize ? (rowVirtualizer.getVirtualItems().map((virtualRow) => {
            const row = rows[virtualRow.index];
            const id = getRowId(row);
            const isSelected = selected?.has(id);
            return (<MemoizedTableRow key={virtualRow.key || virtualRow.index} data-index={virtualRow.index} ref={(el) => rowVirtualizer.measureElement(el)} className={cn("group astryx-table-row flex items-stretch", onRowClick && "cursor-pointer", isSelected && "selected")} style={{
                  ...
                  {selectable && (<OptimizedCell colKey="selection" style={{ width: 40, minWidth: 40, flex: '0 0 40px' }} className="w-10 px-3 text-center sticky left-0 z-20 bg-inherit" onClick={(e) => {
                        e.stopPropagation();
                        onSelect?.(id);
                    }}>
                      <Checkbox checked={isSelected}/>
                    </OptimizedCell>)}
                  {columns.map((col) => (<OptimizedCell key={col.key} colKey={col.key} rowId={id} dataHash={String(row[col.key] ?? '')} style={{
                        width: col.width,
                        minWidth: col.minWidth || (col.width ? undefined : 100),
                        flex: col.width ? `0 0 ${col.width}px` : `1 1 ${col.minWidth || 100}px`,
                        position: col.sticky ? 'sticky' : 'relative',
                        ...
                        background: 'inherit'
                    }} className={cn(col.cellClassName, col.sticky && "border-r border-border/20", col.align === "center" && "text-center", col.align === "right" && "text-right tabular-nums", col.type === "actions" && "sticky right-0 z-20 border-l border-border/20")}>
            ...
            const id = getRowId(row);
            const isSelected = selected?.has(id);
            return (<MemoizedTableRow key={id || `row-${rows.indexOf(row)}`} className={cn("group astryx-table-row flex items-stretch", onRowClick && "cursor-pointer", isSelected && "selected")} style={{ contain: 'layout inline-size' }} onClick={() => onRowClick?.(row)}>
                  {selectable && (<OptimizedCell colKey="selection" style={{ flex: '0 0 40px', width: 40 }} className="w-10 px-3 text-center sticky left-0 z-20 bg-inherit" onClick={(e) => {
                        e.stopPropagation();
                        onSelect?.(id);
                    }}>
                      <Checkbox checked={isSelected}/>
                    </OptimizedCell>)}
                  {columns.map((col) => (<OptimizedCell key={col.key} colKey={col.key} style={{
                        width: col.width,
                        minWidth: col.minWidth || (col.width ? undefined : 100),
                        flex: col.width ? `0 0 ${col.width}px` : `1 1 ${col.minWidth || 100}px`,
                        position: col.sticky ? 'sticky' : 'relative',
