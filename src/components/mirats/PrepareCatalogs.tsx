...
import {
  Server, MapPin, Boxes, Plus, Check, Loader2, Search, X, ChevronDown, ChevronRight,
} from "lucide-react";
...
export function PrepareCatalogs({
  systemMa, onPickSystem, modelMa, onPickModel, viTriParentId, onPickViTriParent,
}: Props) {
  ...
  async function loadAll() {
    const [s, p, m, n, v] = await Promise.all([
      supabase.from("dm_he_thong").select("id, ma, ten, phan_loai_id, dm_phan_loai(ten)").eq("active", true).order("ten"),
      supabase.from("dm_phan_loai").select("id, ma, ten").eq("active", true).order("thu_tu"),
      supabase.from("dm_model").select("id, ma, ten, p_n, dm_nha_san_xuat(ten)").eq("active", true).order("ten"),
      supabase.from("dm_nha_san_xuat").select("id, ma, ten").eq("active", true).order("ten"),
      supabase.from("dm_vi_tri").select("id, ten, parent_id"),
    ]);
    setSystems((s.data ?? []).map((r: any) => ({ id: r.id, ma: r.ma, ten: r.ten, extra: r.dm_phan_loai?.ten })));
    setPhanLoai((p.data ?? []).map((r: any) => ({ id: r.id, ma: r.ma, ten: r.ten })));
    setModels((m.data ?? []).map((r: any) => ({ id: r.id, ma: r.ma, ten: r.ten, extra: [r.p_n, r.dm_nha_san_xuat?.ten].filter(Boolean).join(" · ") })));
    setNsx((n.data ?? []).map((r: any) => ({ id: r.id, ma: r.ma, ten: r.ten })));
    // Cây vị trí có thụt cấp
    const data = v.data ?? [];
    const byId = new Map(data.map((r: any) => [r.id, r]));
    const depth = (r: any): number => { let d = 0, cur = r; while (cur?.parent_id && byId.get(cur.parent_id)) { d++; cur = byId.get(cur.parent_id); if (d > 8) break; } return d; };
    setViTri([...data].sort((a: any, b: any) => (a.ten ?? "").localeCompare(b.ten ?? ""))
  ...
  return (
    <div className="rounded-lg border border-primary/25 bg-primary/[0.03]">
      ...
      >
        ...
        <span className="text-sm font-semibold">Bước 0 — Chuẩn bị danh mục (chọn có sẵn hoặc tạo mới)</span>
        <InfoHint>
          Nếu file thiếu <b>Hệ thống / Vị trí cha / Mẫu</b>, chọn sẵn ở đây — giá trị áp cho mọi dòng bỏ trống.
      ...
      {open && (
        <div className="space-y-4 border-t border-primary/15 p-3">
          ...
          {/* ---- Hệ thống đích ---- */}
          <SystemPicker
            systems={systems} phanLoai={phanLoai} value={systemMa} busy={busy} setBusy={setBusy}
            onPick={onPickSystem} onCreated={loadAll}
          />
          ...
          <div className="space-y-1.5 rounded-md border bg-background/60 p-3">
            <Label className="flex items-center gap-1 text-xs font-semibold">
              ...
              <InfoHint>Phòng mới trong file (vd <i>Tower, Phòng Tài sản</i>) sẽ nằm dưới cấp cha này (vd <i>Đài KSKL Phú Bài</i>).</InfoHint>
            </Label>
            <Select value={viTriParentId || "none"} onValueChange={(v) => onPickViTriParent(v === "none" ? "" : v)}>
              <SelectTrigger className="h-9 text-xs"><SelectValue placeholder="— Không (vị trí mới ở cấp gốc) —" /></SelectTrigger>
              <SelectContent>
                <SelectItem value="none">— Không (cấp gốc) —</SelectItem>
                {viTri.map((v) => <SelectItem key={v.id} value={v.id}>{v.label}</SelectItem>)}
              </SelectContent>
            </Select>
          ...
          {/* ---- Model ---- */}
          <ModelPicker
            models={models} nsx={nsx} value={modelMa} busy={busy} setBusy={setBusy}
            onPick={onPickModel} onCreated={loadAll}
          />
        </div>
      )}
    </div>
...
function SystemPicker({
  systems, phanLoai, value, busy, setBusy, onPick, onCreated,
}: {
  systems: Row[]; phanLoai: Row[]; value: string; busy: boolean;
  setBusy: (b: boolean) => void; onPick: (ma: string) => void; onCreated: () => Promise<void>;
}) {
  ...
  const filtered = useMemo(() => {
    const n = noAccent(q);
    return systems.filter((s) => !n || noAccent(`${s.ten} ${s.ma} ${s.extra ?? ""}`).includes(n)).slice(0, 40);
  }, [systems, q]);

  async function create() {
    if (!ten.trim()) { toast.error("Nhập tên hệ thống"); return; }
    setBusy(true);
    try {
      const ma = randCode("HT");
      const { error } = await supabase.from("dm_he_thong").insert({
        ma, ten: ten.trim(), phan_loai_id: ploai || null, active: true, thu_tu: 0,
      } as any);
      if (error) throw new Error(error.message);
      toast.success(`Đã tạo hệ thống "${ten.trim()}"`);
      await onCreated();
      onPick(ma);
      setCreating(false); setTen(""); setPloai("");
    ...
    <div className="space-y-2 rounded-md border bg-background/60 p-3">
      ...
      {picked ? (
        ...
        <div className="space-y-2 rounded-md border border-dashed p-2.5">
          <Input value={ten} onChange={(e) => setTen(e.target.value)} placeholder="Tên hệ thống (vd: Hệ thống Camera PBA)" className="h-8 text-xs" />
          <Select value={ploai || "none"} onValueChange={(v) => setPloai(v === "none" ? "" : v)}>
            <SelectTrigger className="h-8 text-xs"><SelectValue placeholder="Phân loại (Nhóm 1/2/3)" /></SelectTrigger>
            <SelectContent>
              <SelectItem value="none">— Chưa phân loại —</SelectItem>
              {phanLoai.map((p) => <SelectItem key={p.id} value={p.id}>{p.ten}</SelectItem>)}
            </SelectContent>
          </Select>
          <div className="flex gap-2">
            <Button size="sm" className="h-8 flex-1 text-xs" onClick={create} disabled={busy}>
              {busy ? <Loader2 className="mr-1 h-3.5 w-3.5 animate-spin" /> : <Check className="mr-1 h-3.5 w-3.5" />} Tạo & chọn
            </Button>
            <Button variant="ghost" size="sm" className="h-8 text-xs" onClick={() => setCreating(false)}>Hủy</Button>
          </div>
        </div>
      ) : (
        <>
          <div className="relative">
            ...
            <Input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Tìm hệ thống có sẵn…" className="h-8 pl-7 text-xs" />
          </div>
          {q && (
          ...
          <Button variant="outline" size="sm" className="h-8 w-full text-xs" onClick={() => setCreating(true)}>
...
function ModelPicker({
  models, nsx, value, busy, setBusy, onPick, onCreated,
}: {
  models: Row[]; nsx: Row[]; value: string; busy: boolean;
  setBusy: (b: boolean) => void; onPick: (ma: string) => void; onCreated: () => Promise<void>;
}) {
  const [q, setQ] = useState("");
  const [creating, setCreating] = useState(false);
  ...
  const filtered = useMemo(() => {
    const n = noAccent(q);
    return models.filter((m) => !n || noAccent(`${m.ten} ${m.ma} ${m.extra ?? ""}`).includes(n)).slice(0, 40);
  }, [models, q]);

  async function create() {
    if (!ten.trim()) { toast.error("Nhập tên mẫu"); return; }
    setBusy(true);
    try {
      const ma = randCode("MD");
      const { error } = await supabase.from("dm_model").insert({
        ma, ten: ten.trim(), p_n: pn.trim() || null, nha_san_xuat_id: nsxId || null, active: true, thu_tu: 0,
      } as any);
      if (error) throw new Error(error.message);
      toast.success(`Đã tạo mẫu "${ten.trim()}"`);
      await onCreated();
      onPick(ma);
      setCreating(false); setTen(""); setPn(""); setNsxId("");
    } catch (e) { toast.error((e as Error).message); }
    finally { setBusy(false); }
  }
  ...
  return (
    <div className="space-y-2 rounded-md border bg-background/60 p-3">
      <Label className="flex items-center gap-1.5 text-xs font-semibold">
        <Boxes className="h-3.5 w-3.5 text-primary" /> Model mặc định <span className="font-normal text-muted-foreground">(tùy chọn)</span>
      </Label>
      ...
      {picked ? (
        ...
        <div className="space-y-2 rounded-md border border-dashed p-2.5">
          <Input value={ten} onChange={(e) => setTen(e.target.value)} placeholder="Tên mẫu (vd: P5635E MK II)" className="h-8 text-xs" />
          <Input value={pn} onChange={(e) => setPn(e.target.value)} placeholder="P/N (tùy chọn)" className="h-8 text-xs" />
          <Select value={nsxId || "none"} onValueChange={(v) => setNsxId(v === "none" ? "" : v)}>
            <SelectTrigger className="h-8 text-xs"><SelectValue placeholder="Nhà sản xuất (tùy chọn)" /></SelectTrigger>
            <SelectContent>
              <SelectItem value="none">— Không —</SelectItem>
              {nsx.map((n) => <SelectItem key={n.id} value={n.id}>{n.ten}</SelectItem>)}
            </SelectContent>
          </Select>
          <div className="flex gap-2">
            <Button size="sm" className="h-8 flex-1 text-xs" onClick={create} disabled={busy}>
        ...
        <>
          <div className="relative">
            <Search className="absolute left-2 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-muted-foreground" />
            <Input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Tìm mẫu có sẵn…" className="h-8 pl-7 text-xs" />
          </div>
          {q && (
            <div className="max-h-40 overflow-auto rounded-md border">
              {filtered.length === 0 ? (
