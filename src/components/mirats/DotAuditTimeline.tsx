...
export function DotAuditTimeline({ hangMucId, dotId, limit = 100 }: { hangMucId?: string; dotId?: string; limit?: number }) {
  const { data, isLoading } = useQuery({
    queryKey: ["dbd-audit", hangMucId ?? "*", dotId ?? "*", limit],
    queryFn: async () => {
      let q = supabase.from("dot_bao_duong_audit_log")
        .select("id, action, actor, changes, note, created_at, hang_muc_id, hang_muc:hang_muc_id(dm_he_thong:he_thong_id(ma,ten))")
  ...
  if (!data || data.length === 0) return <div className="text-xs text-muted-foreground">Chưa có thao tác nào.</div>;
        ...
        return (
          <div key={row.id} className="rounded border p-2 text-xs bg-card">
            <div className="flex items-center justify-between gap-2">
              ...
              <span className="text-muted-foreground">{new Date(row.created_at).toLocaleString("vi-VN")}</span>
            </div>
            <div className="mt-1 flex items-center gap-1 text-muted-foreground">
              <User className="h-3 w-3" /><span>{actorName}</span>
            </div>
            {row.note && <div className="mt-1 rounded bg-muted px-2 py-1"><span className="font-medium">Ghi chú:</span> {row.note}</div>}
...
export function DotAuditTimelineHeader() {
  return (
    <div className="flex items-center gap-2 text-sm font-medium">
      <History className="h-4 w-4" />Nhật ký thao tác
    </div>
  );
}