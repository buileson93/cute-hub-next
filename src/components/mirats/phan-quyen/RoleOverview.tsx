...
export function RoleOverview({ stats, loading, error }: RoleOverviewProps) {
  const roleCards = useMemo(() => {
    if (!stats) return [];
    const known = ROLE_ORDER.filter((k) => stats.roles[k]).map((k) => ({
      key: k,
      ...roleMeta[k],
    ...
    return (
      <Card>
        <CardContent className="py-8 text-center text-sm text-destructive">
          Không tải được số liệu phân quyền.
        </CardContent>
      </Card>
    );
  }
        ...
        return (
          <Card key={r.key} className="p-4 transition-all hover:shadow-md group">
            <div className="flex items-center justify-between">
              <span
                className={cn(
                  "inline-flex h-9 w-9 items-center justify-center rounded-lg transition-transform group-hover:scale-110",
                  r.tone,
                )}
              >
              ...
              <div className="text-right">
                ...
                <div className="text-mini text-muted-foreground mt-1">{r.active} hoạt động</div>
              </div>
            </div>
