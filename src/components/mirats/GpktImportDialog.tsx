      ...
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="max-w-4xl max-h-[92vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              Ưu tiên bóc tách nhanh bằng mẫu chuẩn GP-CHK (không tốn AI credit). Nếu PDF là ảnh
              scan hoặc mẫu lạ, hệ thống tự chuyển sang AI. Bạn có thể bấm "Bóc tách lại bằng AI" để
              kiểm tra chéo.
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="rounded-md border border-dashed p-4 flex items-center gap-3">
            ...
            <Button
              variant="outline"
              onClick={() => document.getElementById("gpkt-file-input")?.click()}
            >
            ...
            <div className="rounded-md border border-red-300 bg-red-50 dark:border-red-500/40 dark:bg-red-500/10 p-3 text-sm text-red-900 dark:text-red-100 flex gap-2">
              ...
              <div>
                ...
                <div>
                  ...
                  {dupGpSo.he_thong_ten ? (
                    <>
                      {" "}
                      — hệ thống <b>{dupGpSo.he_thong_ten}</b>
                    </>
                  ) : null}
            ...
            <div className="rounded-md border border-amber-300 bg-amber-50 dark:border-amber-500/40 dark:bg-amber-500/10 p-3 text-sm text-amber-900 dark:text-amber-100 flex gap-2">
              ...
              <div className="space-y-1">
                <div className="font-medium">
                  Hệ thống đã có {dupHeThong.length} GPKT còn hiệu lực
                </div>
                {dupHeThong.slice(0, 5).map((d) => (
                  <div key={d.id}>
                    • <b>{d.gp_so}</b>
                    {d.gp_han ? ` — hết hạn ${d.gp_han}` : ""}
                  </div>
                ))}
            ...
            <div className="rounded-md border bg-muted/30 p-2.5 text-xs flex items-center gap-3 flex-wrap">
              ...
              <span className="text-muted-foreground">
                {needsCheckCount === 0 ? (
                  ...
                  <>
                    <b className="text-amber-600">{needsCheckCount}</b> trường cần bạn kiểm tra
                    (viền vàng).
                  </>
                )}
              </span>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <Field label="Số giấy phép *" meta={perField.gp_so}>
              <Input
                value={fields.gp_so}
                onChange={(e) => updateField("gp_so", e.target.value)}
                placeholder="622/GP-CHK"
                className={cn(inputCn(perField.gp_so))}
              />
            </Field>
            <Field label="Số GP cũ (thay thế)" meta={perField.gp_cu}>
              <Input
                value={fields.gp_cu}
                onChange={(e) => updateField("gp_cu", e.target.value)}
                className={cn(inputCn(perField.gp_cu))}
              />
            </Field>
            <Field label="Ngày cấp" meta={perField.gp_ngay}>
              <Input
                type="date"
                value={fields.gp_ngay}
                onChange={(e) => updateField("gp_ngay", e.target.value)}
                className={cn(inputCn(perField.gp_ngay))}
              />
            </Field>
            <Field label="Ngày hết hạn" meta={perField.gp_han}>
              <Input
                type="date"
                value={fields.gp_han}
                onChange={(e) => updateField("gp_han", e.target.value)}
                className={cn(inputCn(perField.gp_han))}
              />
            </Field>
            <Field label="Tên hệ thống theo GP" meta={perField.ten_he_thong_theo_gp}>
              <Input
                value={fields.ten_he_thong_theo_gp}
                onChange={(e) => updateField("ten_he_thong_theo_gp", e.target.value)}
                className={cn(inputCn(perField.ten_he_thong_theo_gp))}
              />
            </Field>
            ...
            <Field label="Đơn vị" meta={perField.don_vi}>
              <Input
                ...
                className={cn(inputCn(perField.don_vi))}
              />
            </Field>
            <Field label="Trạm" meta={perField.tram}>
              <Input
                ...
                className={cn(inputCn(perField.tram))}
              />
            </Field>
            <Field label="Kiểu thiết bị" meta={perField.kieu_thiet_bi}>
              <Input
                value={fields.kieu_thiet_bi}
                onChange={(e) => updateField("kieu_thiet_bi", e.target.value)}
                className={cn(inputCn(perField.kieu_thiet_bi))}
              />
            </Field>
            <Field label="Số sản xuất" meta={perField.so_san_xuat}>
              <Input
                value={fields.so_san_xuat}
                onChange={(e) => updateField("so_san_xuat", e.target.value)}
                className={cn(inputCn(perField.so_san_xuat))}
              />
            </Field>
            <Field label="Nơi sản xuất" meta={perField.noi_san_xuat}>
              <Input
                value={fields.noi_san_xuat}
                onChange={(e) => updateField("noi_san_xuat", e.target.value)}
                className={cn(inputCn(perField.noi_san_xuat))}
              />
            </Field>
            <Field label="Năm SX" meta={perField.nam_sx_gp}>
              <Input
                value={fields.nam_sx_gp}
                onChange={(e) => updateField("nam_sx_gp", e.target.value)}
                className={cn(inputCn(perField.nam_sx_gp))}
              />
            </Field>
            <Field label="Địa điểm" meta={perField.dia_diem}>
              <Input
                value={fields.dia_diem}
                onChange={(e) => updateField("dia_diem", e.target.value)}
                className={cn(inputCn(perField.dia_diem))}
              />
            </Field>
            <Field label="Mã địa chỉ" meta={perField.ma_dia_chi}>
              <Input
                value={fields.ma_dia_chi}
                onChange={(e) => updateField("ma_dia_chi", e.target.value)}
                className={cn(inputCn(perField.ma_dia_chi))}
              />
            </Field>
            <div className="sm:col-span-2">
              <Field label="Mục đích" meta={perField.muc_dich}>
                <Textarea
                  rows={2}
                  value={fields.muc_dich}
                  onChange={(e) => updateField("muc_dich", e.target.value)}
                  className={cn(inputCn(perField.muc_dich))}
                />
              </Field>
            </div>
            <div className="sm:col-span-2">
              <Field label="Phạm vi" meta={perField.pham_vi}>
                <Textarea
                  rows={2}
                  value={fields.pham_vi}
                  onChange={(e) => updateField("pham_vi", e.target.value)}
                  className={cn(inputCn(perField.pham_vi))}
                />
              </Field>
            </div>
            <div className="sm:col-span-2">
              <Field label="Thành phần theo GP" meta={perField.thanh_phan_theo_gp}>
                <Textarea
                  rows={3}
                  value={fields.thanh_phan_theo_gp}
                  onChange={(e) => updateField("thanh_phan_theo_gp", e.target.value)}
                  className={cn(inputCn(perField.thanh_phan_theo_gp))}
                />
              </Field>
            </div>
            <div className="sm:col-span-2">
              <Field label="Thời gian khai thác" meta={perField.thoi_gian}>
                <Input
                  value={fields.thoi_gian}
                  onChange={(e) => updateField("thoi_gian", e.target.value)}
                  className={cn(inputCn(perField.thoi_gian))}
                />
              </Field>
            </div>
          </div>
          ...
          {!parseM.isPending && fields.gp_so && !dupGpSo && (
            <div className="flex items-center gap-2 text-emerald-600 text-sm">
              ...
              {heThongId && dupHeThong.length === 0
                ? " Hệ thống chưa có GPKT nào còn hiệu lực."
                : ""}
            </div>
          )}
          ...
          <DialogFooter className="gap-2">
            <div className="mr-auto text-xs text-muted-foreground flex items-center gap-2">
              {(parseM.isPending || reparseAiM.isPending) && (
                <>
                  <Loader2 className="h-3 w-3 animate-spin" /> Đang bóc tách…
                </>
              )}
