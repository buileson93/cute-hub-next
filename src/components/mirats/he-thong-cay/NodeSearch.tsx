...
export function NodeSearch({
  ...
  const results = useMemo(() => {
    ...
    for (const it of list)
      if (it.kind === "ht") childHt.add(`${it.plId}|${it.lvId ?? ""}|${it.nhMa ?? ""}`);
    list = list.filter(
      (it) => !(it.kind === "nh" && childHt.has(`${it.plId}|${it.lvId ?? ""}|${it.ma}`)),
    );
    ...
    <div ref={boxRef} className={cn("relative", containerClassName)}>
      <div className="relative">
        ...
        <Input
          ...
          placeholder="Tìm phân loại, hệ thống, tài sản…"
            ...
            return (
              ...
              >
                ...
                <span className="min-w-0 flex-1 truncate">
                  {it.code && it.kind !== "tb" && it.kind !== "tp" && (
                    <span className="mr-1 font-mono text-xs opacity-60">{it.code}</span>
                  )}
                  {it.label}
                </span>
