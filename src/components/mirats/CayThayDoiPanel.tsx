    ...
    <Sheet open={open} onOpenChange={(o) => !o && onClose()}>
      <SheetContent side="right" className="flex w-full flex-col sm:max-w-lg">
        <SheetHeader>
          ...
          <SheetDescription>
            Nhật ký chỉnh sửa sơ đồ hệ thống.{" "}
            {isAdmin
              ? "Duyệt, từ chối hoặc hoàn tác về dữ liệu cũ."
              : "Admin sẽ duyệt các thay đổi bạn gửi."}
          </SheetDescription>
        </SheetHeader>
          ...
          {isLoading ? (
            ...
            <ul className="space-y-2">
              {rows.map((c) => {
                ...
                const st = TRANG_THAI[c.trang_thai] ?? {
                  label: c.trang_thai,
                ...
                return (
                  <li key={c.id} className="rounded-md border p-3">
                    <div className="flex items-center justify-between gap-2">
                      ...
                      <span className={cn("rounded border px-1.5 py-0.5 text-mini", st.badge)}>
                        {st.label}
                      </span>
                    </div>
                    ...
                    {htTen && (
                      <div className="mt-0.5 text-xs">
                        Hệ thống: <span className="font-medium">{htTen}</span>
                      </div>
                    )}
