...
import { Download, Loader2, ChevronDown, ChevronRight, Search, Layers, Info } from "lucide-react";
...
export function AllInOneExportPanel() {
  ...
  async function doExport() {
    ...
    try {
      ...
      toast.success(
        compact
          ? "Đã tải mẫu rút gọn theo phạm vi đã chọn"
    ...
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center gap-2 text-base">
          ...
          <InfoHint>
            Mỗi lớp chọn <b>Tất cả</b> / <b>Không xuất</b> / <b>Chọn cụ thể</b> (ví dụ chỉ Nhóm
            1/2/3). Chọn Phân loại/Nhóm hệ thống thì Hệ thống & Tài sản tự thu hẹp theo; danh mục
            liên quan (NSX, Mẫu, Loại…) tự kèm đầy đủ.
          </InfoHint>
        </CardTitle>
      </CardHeader>
        ...
        {loading ? (
          ...
          <>
                        ...
                        <div className="flex items-center gap-2">
                          <div className="relative flex-1">
                            ...
                            <Input
                              ...
                              onChange={(e) =>
                                setSearch((s) => ({ ...s, [l.table]: e.target.value }))
                              }
            ...
            <label className="flex cursor-pointer items-start gap-2 rounded-md border border-primary/20 bg-primary/5 p-2.5 text-xs">
              <Checkbox
                checked={autoDeps}
                onCheckedChange={(v) => setAutoDeps(!!v)}
                className="mt-0.5"
              />
              <span className="text-muted-foreground">
                <b className="text-foreground">Tự động kèm danh mục liên quan đầy đủ</b> — khi xuất
                Hệ thống/Tài sản, các Nhà sản xuất, Model, Chủng loại… mà chúng dùng sẽ được xuất
                kèm để nhập lại không bị thiếu liên kết.
              </span>
            </label>
            ...
            <label className="flex cursor-pointer items-start gap-2 rounded-md border border-amber-400/30 bg-amber-50/60 p-2.5 text-xs dark:bg-amber-500/10">
              ...
              />
              <span className="text-muted-foreground">
                <b className="text-foreground">Mẫu rút gọn</b> — chỉ giữ các cột <b>bắt buộc</b> và{" "}
                <b>hay dùng</b> (Mã, Tên, Hệ thống, Lắp vào vị trí, Serial, Model…). Giảm số cột
                phải nhập; các trường còn lại vẫn khai được bằng mẫu đầy đủ.
              </span>
            </label>
