...
import { History, Activity, Move, RefreshCw } from "lucide-react";
          ...
          <TableBody>
            {isLoading ? (
              ...
              <TableRow key={i}>
                ...
                <TableCell className="text-xs text-muted-foreground">{m.don_vi}</TableCell>
              </TableRow>
            ))}
          </TableBody>
        </Table>
        ...
        <Table>
          ...
          <TableBody>
            {isLoading ? (
              ...
              <TableRow key={i}>
                <TableCell className="text-xs font-mono">{new Date(h.tu_ngay).toLocaleDateString("vi-VN")}</TableCell>
                <TableCell className="text-xs font-mono">{h.den_ngay ? new Date(h.den_ngay).toLocaleDateString("vi-VN") : "Hiện tại"}</TableCell>
                <TableCell>{donViTenMap[h.don_vi_id] || h.don_vi_id}</TableCell>
                <TableCell>
                  <Badge variant={!h.den_ngay ? "default" : "secondary"}>
                    {!h.den_ngay ? "Đang sử dụng" : "Đã trả"}
                  </Badge>
                </TableCell>
              </TableRow>
            ))}
        ...
        <Table>
          ...
          <TableBody>
            {isLoading ? (
              ...
              <TableRow><TableCell colSpan={4} className="text-center py-4 text-muted-foreground">Chưa có bản ghi vòng đời.</TableCell></TableRow>
            ) : lifecycle.map((l, i) => (
              <TableRow key={i}>
                <TableCell className="text-xs font-mono">{new Date(l.created_at).toLocaleString("vi-VN")}</TableCell>
                <TableCell>
                  <Badge variant="outline">{statusMap?.get(l.trang_thai_id) || l.trang_thai_id}</Badge>
                </TableCell>
                <TableCell className="text-sm italic">{l.ghi_chu || "—"}</TableCell>
                <TableCell className="text-xs">{l.created_by || "Hệ thống"}</TableCell>
              </TableRow>
            ))}
          </TableBody>
        </Table>
