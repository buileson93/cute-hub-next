  ...
  return (
    <Tabs defaultValue="timeline" className="w-full">
      ...
      <TabsContent value="baotri" className="mt-4 space-y-2">
        {baoTri.length === 0 && (
          <p className="py-8 text-center text-sm text-muted-foreground">Chưa có phiếu bảo dưỡng.</p>
        )}
        {baoTri.map((e) => (
          <EventRow
            key={e.ma_bao_tri || e.id}
      ...
      <TabsContent value="suco" className="mt-4 space-y-2">
        {suCo.length === 0 && (
          <p className="py-8 text-center text-sm text-muted-foreground">Không có sự cố ghi nhận.</p>
        )}
        {suCo.map((e) => (
          <EventRow
            key={e.ma_su_co || e.id}
      ...
      <TabsContent value="thaythe" className="mt-4 space-y-2">
        {hongHoc.length === 0 && (
          <p className="py-8 text-center text-sm text-muted-foreground">
            Chưa có ghi nhận hỏng hóc / thay thế.
          </p>
        )}
        {hongHoc.map((e) => (
          <EventRow
            key={e.ma_hong_hoc || e.id}
          ...
          it.kind === "bt"
            ...
            : it.kind === "sc"
              ...
              : it.kind === "hh"
                ...
                : "bg-info";
        const iconColor =
          it.kind === "hh" || it.kind === "sc" ? "text-foreground" : "text-primary-foreground";
        return (
          <li key={i} className="relative mb-5 last:mb-0">
            <span
              className={`absolute -left-[31px] flex h-6 w-6 items-center justify-center rounded-full ring-4 ring-background shadow-sm ${color}`}
            >
