    ...
    <div className="space-y-4">
      {data.length === 0 ? (
        <p className="py-8 text-center text-sm text-muted-foreground italic">
          Chưa có lịch sử cấp phát cho tài sản này.
        </p>
      ) : (
          ...
          {data.map((row) => {
            ...
            return (
              <li key={row.id} className="relative">
                <span
                  className={`absolute -left-[35px] flex h-7 w-7 items-center justify-center rounded-full ring-4 ring-background shadow-sm ${isCapPhat ? "bg-primary" : "bg-muted"}`}
                >
                  {isCapPhat ? (
                ...
                <div className="rounded-lg border p-4 bg-card shadow-sm hover:shadow-md transition-shadow">
                  <div className="flex flex-wrap items-center justify-between gap-2 mb-2">
                    <div className="flex items-center gap-2">
                      <Badge
                        variant={isCapPhat ? "default" : "secondary"}
                        className={isCapPhat ? "bg-primary text-primary-foreground" : ""}
                      >
