  ...
  return (
    <div className="grid gap-6 lg:grid-cols-3">
      ...
      <div className="space-y-4 lg:col-span-1">
        <Card className="astryx-card">
          <CardHeader className="p-4 pb-0">
            <CardTitle className="astryx-text-label">Thông tin tài sản</CardTitle>
          </CardHeader>
          <CardContent className="p-4 space-y-4">
            {refInfo?.modelImg && (
              <div className="flex items-center justify-center overflow-hidden rounded-md border bg-muted/40 p-2">
                <img
                  src={refInfo.modelImg}
                  alt={refInfo.model || "Model"}
            ...
            <InfoRow
              icon={FileText}
              label="Mã tài sản Bravo"
              value={<span className="font-mono">{tb._maBravo || "—"}</span>}
              field="ma_tai_san_bravo"
              canEdit={canEdit}
              editor={editor}
              ma={ma}
              currentValue={tb._maBravo}
            />
            <InfoRow
              icon={Package}
              label="Model"
              value={
                refInfo?.model || tb.model ? (
                  <span className="inline-flex flex-wrap items-center gap-1.5">
                    <Link
                      to="/danh-muc/model"
                      search={{ q: (refInfo?.model || tb.model) as string }}
              ...
              editor={editor}
              ma={ma}
              currentValue={tb.model}
            />
            <InfoRow
              icon={FileText}
              label="Serial"
              value={<span className="font-mono">{tb.serial || "—"}</span>}
              field="ma_serial"
              canEdit={canEdit}
              editor={editor}
              ma={ma}
              currentValue={tb.serial}
            />
            <InfoRow
              icon={Factory}
              label="Nhà sản xuất"
              value={
                refInfo?.nhaSanXuat || tb.nha_san_xuat ? (
                  <Link
                    ...
                    search={{ q: (refInfo?.nhaSanXuat || tb.nha_san_xuat) as string }}
              ...
              value={
                refInfo?.nhaCungCap || tb.nha_cung_cap ? (
                  <Link
                    ...
                    search={{ q: (refInfo?.nhaCungCap || tb.nha_cung_cap) as string }}
                    className="inline-flex items-center gap-1 text-primary transition-colors hover:underline"
                  >
                    {refInfo?.nhaCungCap || tb.nha_cung_cap}
                    <ExternalLink className="h-3 w-3" />
              ...
              <InfoRow
                ...
                value={
                  <Link
                    ...
                    search={{ q: (tb._loaiTbTen || tb.loai) as string }}
            ...
            <InfoRow
              ...
              value={
                sysName ? (
                  <Link
                    to="/danh-muc/he-thong"
                    search={{ f_id: tb._htId || tb.he_thong }}
                    className="inline-flex items-center gap-1 text-primary hover:underline"
                  >
                    {sysName}
                    <ExternalLink className="h-3 w-3" />
            ...
            <InfoRow
              ...
              value={
                tb.vi_tri ? (
                  <Link
                    to="/danh-muc/vi-tri"
                    search={{ f_id: tb._viTriId || tb.vi_tri }}
                    className="inline-flex items-center gap-1 text-primary hover:underline"
                  >
                    {tb.vi_tri}
                    <ExternalLink className="h-3 w-3" />
      ...
      <div className="space-y-4 lg:col-span-2">
        <Card className="astryx-card">
          <CardHeader className="p-4 pb-0">
            <CardTitle className="astryx-text-label">Vòng đời & Khai thác</CardTitle>
          </CardHeader>
          <CardContent className="grid gap-6 md:grid-cols-2 p-4">
            <div className="space-y-3 text-sm">
              <InfoRow
                ...
                editor={editor}
                ma={ma}
                currentValue={tb.ngay_mua}
              />
              <InfoRow
                icon={Calendar}
                label="Năm khai thác"
                value={tb._namKhaiThac ? String(tb._namKhaiThac) : tb.ngay_dua_vao_su_dung || "—"}
                field="nam_dua_vao_khai_thac"
                canEdit={canEdit}
                editor={editor}
                ma={ma}
                currentValue={tb._namKhaiThac || tb.ngay_dua_vao_su_dung}
              />
              <InfoRow
                icon={ShieldCheck}
                label="Hạn bảo hành"
                value={tb.han_bao_hanh || "—"}
                field="han_bao_hanh"
                canEdit={canEdit}
                editor={editor}
                ma={ma}
                currentValue={tb.han_bao_hanh}
              />
            </div>
