...
export function OperationDialog({ mode, target, onClose, onSuccess }: OperationDialogProps) {
  ...
  const [lastOpResult, setLastOpResult] = useState<{ type: string; id: string; ma: string } | null>(
    null,
  );
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (mode === "lap") {
        ...
        if (!current) {
          toast.error("Không có tài sản để tháo");
          return;
        }
        if (!destLocationId) {
          toast.error("Vui lòng chọn vị trí đích (kho/xưởng)");
          return;
        }
        ...
        if (!pickedAssetId) {
          toast.error("Vui lòng chọn tài sản mới");
          return;
        }
        if (!destLocationId) {
          toast.error("Vui lòng chọn vị trí đích cho tài sản cũ");
          return;
        }
      ...
      const targetMa =
        mode === "thao"
          ? current?.ma_thiet_bi
          : allAssets.find((a) => a.id === pickedAssetId)?.ma_thiet_bi;
        ...
        <div className="rounded-md border bg-muted/40 p-3 text-xs space-y-1">
          <div className="flex justify-between">
            ...
            <span className="font-medium">
              {target?.maThanhPhan} · {target?.tenThanhPhan}
            </span>
          </div>
          {current && (
