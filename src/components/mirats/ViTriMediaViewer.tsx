  ...
  const mediaQ = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("vi_tri_media")
        .select(
          "id,vi_tri_ma,loai,ten_tep,duong_dan,mo_ta,kich_thuoc,content_type,created_by,created_at,vi_do,kinh_do,do_chinh_xac,chup_luc",
        )
      ...
      const { data: signed } = await storage.from(BUCKET).createSignedUrls(
  ...
  const uploadFiles = async (files: FileList | null, forceLoai?: MediaLoai) => {
    ...
    try {
      for (const file of Array.from(files)) {
        ...
        else if (IMG_EXT.test(ext) || file.type.startsWith("image/"))
          loai = await detectImageLoai(file);
        else {
      ...
      <DialogContent className="flex max-h-[92vh] w-[96vw] max-w-4xl flex-col gap-0 overflow-hidden p-0">
        ...
        <div className="flex flex-wrap items-center gap-2 border-b bg-background p-3">
          {laDiDong ? (
            ...
            <span className="flex items-center gap-1.5 text-xs text-muted-foreground">
              <Smartphone className="h-4 w-4" /> Chỉ chụp ảnh check-in trên điện thoại có camera &
              GPS. Trên máy tính chỉ xem.
            </span>
          )}
          ...
          <input
            ref={modelRef}
            type="file"
            accept=".glb,.gltf,.usdz,.ply,.obj,.las,.laz,.e57,.xyz,.fbx,.stl"
            multiple
            className="hidden"
            ...
            {mediaQ.isLoading ? (
              ...
              diemGps.length === 0 ? (
                <div className="flex flex-col items-center gap-2 py-12 text-center text-sm text-muted-foreground">
                  ...
                  <p className="text-xs">
                    Chụp ảnh check-in trên điện thoại (bắt buộc GPS) để xuất hiện trên bản đồ.
                  </p>
                </div>
              ) : (
                <CheckinMap
              ...
              <div className="flex flex-col items-center gap-2 py-12 text-center text-sm text-muted-foreground">
                ...
                <p className="text-xs">
                  {laDiDong ? "Chụp ảnh check-in" : "Dùng điện thoại để chụp ảnh check-in"} kèm GPS.
                  Hỗ trợ ảnh thường, ảnh 360° (tự nhận diện tỉ lệ 2:1) và mô hình 3D/LiDAR xuất từ
                  Polycam (GLB/USDZ).
                </p>
              </div>
            ) : (
              ...
              <div className="flex items-center gap-1.5">
                {(active.loai === "anh" || active.loai === "pano360") && (
                  <Button
                    size="sm"
                    variant="outline"
                    className="h-8 border-primary/20 text-primary hover:bg-primary/5 hover:text-primary/90"
                ...
                is3DViewable(active) ? (
                  ...
                  <div className="flex h-full flex-col items-center justify-center gap-3 p-6 text-center text-sm text-white/80">
                    ...
                    <p className="text-xs text-white/60">
                      Để xem trực tiếp trên web, hãy xuất mô hình sang <b>GLB</b> hoặc <b>USDZ</b>{" "}
                      (Polycam hỗ trợ sẵn).
                    </p>
                    <Button
                      size="sm"
                      ...
                      asChild
                    >
                      <a href={active.url} download={active.ten_tep}>
                ...
                {active.vi_do != null && active.kinh_do != null && (
                  ...
                  >
                    ...
                    {active.do_chinh_xac != null && (
                      <span className="text-muted-foreground">
                        {" "}
                        · ±{Math.round(active.do_chinh_xac)}m
                      </span>
                    )}
                  </a>
                )}
