  ...
  return (
    <div className="space-y-4">
        ...
        <Select value={f.kind} onValueChange={(v) => onChange({ kind: v })}>
          ...
          <SelectContent>
            {FIELD_KIND_GROUPS.map((g) => (
              <div key={g.label}>
                <div className="px-2 py-1 text-mini font-semibold uppercase text-muted-foreground">
                  {g.label}
                </div>
                {g.items.map((it) => (
      ...
      <div className="grid grid-cols-2 gap-2">
        ...
        <div className="col-span-2">
          ...
          <Input
            value={f.key}
            onChange={(e) =>
              onChange({ key: e.target.value.replace(/[^a-z0-9_]/gi, "_").toLowerCase() })
            }
            maxLength={40}
        ...
        <div className="col-span-2">
          ...
          <Select
            value={String(f.col_span)}
            onValueChange={(v) => onChange({ col_span: Number(v) })}
          >
            <SelectTrigger>
              <SelectValue />
            </SelectTrigger>
        ...
        <div className="col-span-2">
          ...
          <Input
            value={f.nhom ?? ""}
            onChange={(e) => onChange({ nhom: e.target.value })}
            placeholder="vd: Thông số kỹ thuật"
          />
        </div>
      </div>
        ...
        <div className="rounded-md border p-2">
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            <div className="col-span-2">
              ...
              <Input
                value={f.tieu_chuan ?? ""}
                onChange={(e) => onChange({ tieu_chuan: e.target.value })}
                placeholder="vd: ICAO Annex 10"
              />
            </div>
            <div>
              ...
              <Input
                ...
                value={f.min_value ?? ""}
                onChange={(e) =>
                  onChange({ min_value: e.target.value === "" ? null : Number(e.target.value) })
                }
              />
            </div>
            <div>
              ...
              <Input
                ...
                value={f.max_value ?? ""}
                onChange={(e) =>
                  onChange({ max_value: e.target.value === "" ? null : Number(e.target.value) })
                }
              />
            </div>
          </div>
            ...
            <Button
              ...
              onClick={() =>
                onChange({
                  ratings: [
                    ...(f.ratings ?? []),
                    { value: String((f.ratings?.length ?? 0) + 1), label: "Mức mới", color: null },
                  ],
                })
              }
            >
            ...
            {(f.ratings ?? []).map((r, i) => (
              <div key={i} className="grid grid-cols-[60px_1fr_60px_28px] gap-1">
                <Input
                  ...
                  onChange={(e) => {
                    const cp = [...(f.ratings ?? [])];
                    cp[i] = { ...cp[i], value: e.target.value };
                    onChange({ ratings: cp });
                  }}
            ...
            <Button
              ...
              onClick={() =>
                onChange({
                  columns: [
                    ...(f.columns ?? []),
                    { key: `col_${(f.columns?.length ?? 0) + 1}`, label: "Cột mới", kind: "text" },
                  ],
                })
              }
            >
              ...
              <div key={i} className="rounded border p-1.5">
                ...
                <div className="mt-1 grid grid-cols-2 gap-1">
                  ...
                  <Input
                    ...
                    onChange={(e) => {
                      const cp = [...(f.columns ?? [])];
                      cp[i] = { ...cp[i], unit: e.target.value };
                      onChange({ columns: cp });
                    }}
                    className="h-7 text-xs"
                    placeholder="Đơn vị (nếu có)"
                  />
                </div>
              </div>
            ))}
      ...
      {isComputed && (
        <div className="rounded-md border p-2">
          ...
          <Input
            ...
            placeholder="vd: {gia_tri_do} * 0.8"
            className="font-mono text-xs"
          />
          <p className="mt-1 text-mini text-muted-foreground">
            Dùng &#123;key_của_trường&#125; để tham chiếu.
          </p>
        </div>
      )}
      ...
      <Collapsible open={advOpen} onOpenChange={setAdvOpen} className="rounded-md border">
        ...
        >
          <ChevronDown
            className={`h-3 w-3 transition-transform ${advOpen ? "rotate-180" : ""}`}
            aria-hidden
          />
        ...
        <CollapsibleContent className="space-y-3 border-t p-2">
          <div className="rounded-md border p-2">
            ...
            {f.visible_if ? (
              <div className="space-y-1">
                <div className="grid grid-cols-[1fr_70px] gap-1">
                  <Select
                    ...
                    onValueChange={(v) =>
                      onChange({ visible_if: { ...f.visible_if!, field_key: v } })
                    }
                  >
                    <SelectTrigger className="h-7 text-xs">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    onChange({ visible_if: { ...f.visible_if!, value: e.target.value } })
                  }
                  className="h-7 text-xs"
                ...
                <Button
                  size="sm"
                  variant="ghost"
                  className="h-6 w-full text-xs text-rose-600"
                  onClick={() => onChange({ visible_if: null })}
                >
                  Bỏ điều kiện
                </Button>
              </div>
              ...
              <Button
                ...
                onClick={() =>
                  onChange({
                    visible_if: { field_key: otherFields[0]?.key ?? "", op: "eq", value: "" },
                  })
                }
            ...
            <div className="rounded-md border p-2">
              ...
              {f.required_if ? (
                <div className="space-y-1">
                  <div className="grid grid-cols-[1fr_70px] gap-1">
                    <Select
                      value={f.required_if.field_key}
                      onValueChange={(v) =>
                        onChange({ required_if: { ...f.required_if!, field_key: v } })
                      }
                    >
                      <SelectTrigger className="h-7 text-xs">
                        <SelectValue placeholder="Trường…" />
                  ...
                  <Input
                    value={String(f.required_if.value ?? "")}
                    onChange={(e) =>
                      onChange({ required_if: { ...f.required_if!, value: e.target.value } })
                    }
                    className="h-7 text-xs"
                    placeholder="Giá trị so sánh"
                  />
                  <Button
                    ...
                    variant="ghost"
                    className="h-6 w-full text-xs text-rose-600"
                    onClick={() => onChange({ required_if: null })}
                  >
                    Bỏ điều kiện
                  </Button>
                </div>
                ...
                <Button
                  ...
                  onClick={() =>
                    onChange({
                      required_if: { field_key: otherFields[0]?.key ?? "", op: "eq", value: "" },
                    })
                  }
                  disabled={otherFields.length === 0}
                >
          ...
          {!isDisplay && (
            <div className="rounded-md border p-2">
              ...
              <Input
                value={f.constraint_formula ?? ""}
                onChange={(e) => onChange({ constraint_formula: e.target.value })}
                className="font-mono text-xs"
                placeholder="vd: {gia_tri_sau} >= {gia_tri_truoc}"
              />
              <Input
                value={f.constraint_message ?? ""}
                onChange={(e) => onChange({ constraint_message: e.target.value })}
                className="mt-1 text-xs"
                placeholder="Thông báo khi vi phạm (tuỳ chọn)"
                maxLength={200}
              />
              <p className="mt-1 text-mini text-muted-foreground">
      ...
      <div className="text-mini text-muted-foreground">
        <Badge variant="outline" className="mr-1 font-mono text-mini">
          kind={f.kind}
        </Badge>
