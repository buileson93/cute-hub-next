...
function idOf(x: string | INode): string {
...
export function SystemInternalGraph({
  ...
  const fgData = useMemo(() => {
    ...
    const links: ILink[] = doThi.links.map((l) => ({
      id: l.id,
      source: l.khe_nguon_id,
      target: l.khe_dich_id,
      mau: l.mau_sac,
      loai_ten: l.loai_ten,
    }));
    return { nodes, links };
  ...
  const endDrag = useCallback(() => {
    ...
    if (dup) {
      toast.info("Đã có liên kết giữa hai thành phần này.");
      return;
    }
    ...
    <div className="overflow-hidden rounded-lg border">
      ...
      <Dialog open={!!connect} onOpenChange={(o) => !o && setConnect(null)}>
        <DialogContent className="sm:max-w-md">
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1.5">
                ...
                <Input
                  value={gdNguon}
                  onChange={(e) => setGdNguon(e.target.value)}
                  placeholder="VD: LAN1"
              ...
              <div className="space-y-1.5">
                ...
                <Input
                  value={gdDich}
                  onChange={(e) => setGdDich(e.target.value)}
                  placeholder="VD: WAN"
                  className="h-8 text-xs"
                />
              </div>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setConnect(null)}>
              Huỷ
            </Button>
      ...
      <AlertDialog open={!!delLink} onOpenChange={(o) => !o && setDelLink(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              {delLink
                ? `${nodeById.get(delLink.khe_nguon_id)?.ten ?? "?"} → ${nodeById.get(delLink.khe_dich_id)?.ten ?? "?"}${delLink.loai_ten ? ` (${delLink.loai_ten})` : ""}`
                : ""}
    ...
    <div className="flex h-full flex-col items-center justify-center gap-3 p-6 text-center text-sm text-muted-foreground">
      ...
      <div>
        ...
        <div className="mt-1 text-xs">
          Đây là bản ghi hệ thống rỗng trong CSDL — chưa có dòng nào trong{" "}
          <span className="font-mono">he_thong_thanh_phan</span>. Vào chi tiết hệ thống và bấm{" "}
          <b>Khai thêm thành phần</b> để bắt đầu.
        </div>
      </div>

      <div className="flex flex-wrap items-center justify-center gap-2">
        <Button size="sm" variant="default" asChild>
          <Link to="/he-thong/$id" params={{ id: systemId }}>
            Mở chi tiết hệ thống
          </Link>
        </Button>
      </div>
      ...
      {coDuLieu.length > 0 && (
        <div className="mt-2 w-full max-w-md rounded-md border bg-background p-3 text-left">
          <div className="mb-2 text-xs">
            Có <b>{coDuLieu.length}</b> hệ thống tên gần giống <b>đã có thành phần</b> — có thể bạn
            muốn xem một trong số này:
          </div>
                ...
                <span className="flex shrink-0 items-center gap-2">
                  ...
                  {onJumpSystem ? (
                    ...
                    <Button size="sm" variant="outline" asChild>
                      <Link to="/he-thong/$id" params={{ id: s.id }}>
                        Mở
                      </Link>
                    </Button>
                  )}
