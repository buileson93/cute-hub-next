    ...
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent className="sm:max-w-xl">
        <SheetHeader>
          ...
          <SheetDescription>
            Danh sách máy tính và bản quyền phần mềm gán cho nhân viên{" "}
            <strong>{nhanVienTen}</strong>.
          </SheetDescription>
        </SheetHeader>
          ...
          {isLoading ? (
            ...
            <div className="py-20 text-center space-y-3">
              ...
              <div className="text-sm text-muted-foreground">
                Nhân viên này chưa được gán tài sản máy tính nào.
              </div>
            </div>
          ) : (
                    ...
                    {asset.software.length === 0 ? (
                      ...
                      <div className="space-y-3">
                        {asset.software.map((sw: any) => {
                          const status = sw.ngay_het_han
                            ? trangThaiBanQuyen(sw.ngay_het_han)
                            : "perpetual";
                          return (
                            <div
                              key={sw.id}
                              className="group relative rounded-lg border p-3 hover:bg-primary/5 transition-colors"
                            >
