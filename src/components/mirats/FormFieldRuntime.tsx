  ...
  switch (f.kind) {
    ...
    case "number":
      return (
        <div className="flex items-stretch gap-2">
          ...
          />
          {f.unit && (
            <span className="flex items-center rounded-md border bg-muted px-2 text-xs text-muted-foreground">
              {f.unit}
            </span>
          )}
        </div>
      ...
      return (
        <div className="space-y-1">
          {(f.options ?? ["Đạt", "Không đạt"]).map((o) => (
            <label
              key={o}
              className="astryx-text-body flex items-center gap-2 text-xs cursor-pointer hover:text-foreground transition-colors py-0.5"
            >
              <input
                type="radio"
                name={`r_${f.key}`}
          ...
          <div
            className={`astryx-text-label flex items-center justify-center rounded-md border text-[9px] font-bold tracking-wider px-2 ${
              status === "pass"
                ...
                : status === "fail"
                  ? "bg-rose-50/50 border-rose-200/50 text-rose-700 dark:bg-rose-950/20 dark:text-rose-300"
      ...
      return (
        <div className="grid grid-cols-2 gap-2">
          <div className="astryx-surface border-border/50 p-2.5 bg-card/50">
            ...
            <Input
              value={v.before ?? ""}
              onChange={(e) => set({ ...v, before: e.target.value })}
              disabled={disabled}
            />
          </div>
          <div className="astryx-surface border-border/50 p-2.5 bg-card/50">
            ...
            <Input
              value={v.after ?? ""}
              onChange={(e) => set({ ...v, after: e.target.value })}
              disabled={disabled}
            />
          </div>
        </div>
      ...
      return (
        <div className="flex items-center gap-1">
          <Input
            type="number"
            placeholder="giờ"
            className="w-20 astryx-number"
            value={v.h ?? ""}
            onChange={(e) => set({ ...v, h: e.target.value })}
            disabled={disabled}
          />
          ...
          <Input
            ...
            className="w-20 astryx-number"
            value={v.m ?? ""}
            onChange={(e) => set({ ...v, m: e.target.value })}
            disabled={disabled}
          />
        </div>
      );
      ...
      return (
        <div className="flex items-center gap-1">
          <Input
            value={(value as string) ?? ""}
            onChange={(e) => set(e.target.value)}
            className="astryx-number"
            placeholder={ph ?? "vd: 21.028, 105.804"}
            disabled={disabled}
          />
          <Button
            type="button"
              ...
              {rows.map((r, i) => (
                <tr key={i}>
                  ...
                  <td className="border-t px-1 text-right">
                    <button
                      type="button"
                      onClick={() => set(rows.filter((_, x) => x !== i))}
                      disabled={disabled}
              ...
              {rows.length === 0 && (
                <tr>
                  <td
                    ...
                    className="border-t p-2 text-center text-muted-foreground"
                  >
                    Chưa có dòng
                  </td>
                </tr>
    ...
    case "signature": {
      ...
      if (signers.length > 0) {
        ...
        return (
          <MultiSignatureFlow value={slots} onChange={set} signers={signers} disabled={disabled} />
        );
      }
      return (
        <SignaturePad
...
export function useGridRows(fields: CompiledField[]) {
  return useMemo(() => {
    ...
    for (const f of fields) {
      ...
      if (used + effective > 3 && cur.length > 0) {
        rows.push(cur);
        cur = [];
        used = 0;
      }
