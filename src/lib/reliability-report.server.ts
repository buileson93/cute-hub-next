...
export async function buildReliabilityMessage(admin: SupabaseClient, type: ReportType): Promise<string> {
  const { from, to, label } = periodRange(type);

  const { data: rows } = await admin
    .from("su_co")
    .select("id,muc_do,trang_thai,snapshot_he_thong,snapshot_don_vi,ngay_phat_hien,ngay_ket_thuc,created_at")
    .gte("created_at", from)
    .lte("created_at", to)
    .limit(5000);

  const list = (rows ?? []) as Array<{
    id: string; muc_do: string | null; trang_thai: string | null;
    snapshot_he_thong: string | null; snapshot_don_vi: string | null;
    ngay_phat_hien: string | null; ngay_ket_thuc: string | null; created_at: string;
  }>;

  const total = list.length;
  const closed = list.filter((r) => r.ngay_ket_thuc).length;
  const open = total - closed;
  const critical = list.filter((r) => (r.muc_do ?? "").toLowerCase().includes("nghiem") || (r.muc_do ?? "").toLowerCase() === "cao").length;

  // MTTR (phút) trên sự cố đã đóng
  const mttrs = list
    .filter((r) => r.ngay_ket_thuc && r.ngay_phat_hien)
    .map((r) => (new Date(r.ngay_ket_thuc as string).getTime() - new Date(r.ngay_phat_hien as string).getTime()) / 60000)
  ...
  lines.push(`• Tổng sự cố: <b>${total}</b> · Đã đóng: <b>${closed}</b> · Đang xử lý: <b>${open}</b>`);
  lines.push(`• Nghiêm trọng/cao: <b>${critical}</b> · MTTR trung bình: <b>${fmtMinutes(mttr)}</b>`);
  if (top.length) {
    lines.push("");
