...
export const getHeThongLyLich = defineTool({
  ...
  description: "Snapshot toàn diện 1 hệ thống: thông tin, thành phần con, GPKT hiện hành, sự cố 30 ngày.",
  ...
  handler: async ({ he_thong_id }, ctx) => {
    ...
    const [ht, tp, gpkt, su_co] = await Promise.all([
      ...
      sb.from("he_thong_thanh_phan").select("id, ma_thanh_phan, ten_thanh_phan, loai, trang_thai").eq("he_thong_id", he_thong_id),
      sb.from("giay_phep_khai_thac").select("id, so_giay_phep, co_quan_cap, ngay_cap, ngay_het_han, trang_thai").eq("he_thong_id", he_thong_id).order("ngay_het_han", { ascending: false }).limit(5),
      sb.from("su_co").select("id, ma_su_co, hien_tuong, muc_do, trang_thai, ngay_phat_hien").eq("he_thong_id", he_thong_id).gte("ngay_phat_hien", new Date(Date.now() - 30 * 86400_000).toISOString().slice(0, 10)).order("ngay_phat_hien", { ascending: false }).limit(20),
    ]);
    if (ht.error) return errResult(ht.error.message);
    return textResult({ he_thong: ht.data, thanh_phan: tp.data ?? [], gpkt: gpkt.data ?? [], su_co_30_ngay: su_co.data ?? [] });
  },
});
...
export const listThanhPhanByHeThong = defineTool({
  ...
  inputSchema: { he_thong_id: z.string().uuid(), limit: z.number().int().min(1).max(200).default(100) },
  annotations: { readOnlyHint: true, idempotentHint: true, openWorldHint: false },
...
export const listDotBaoDuong = defineTool({
  ...
  handler: async ({ nam, don_vi_id, trang_thai, limit }, ctx) => {
    const auth = requireAuth(ctx); if (auth) return auth;
    let q = supabaseForUser(ctx).from("dot_bao_duong").select("*").order("nam", { ascending: false }).limit(limit);
...
export const getDotBaoDuong = defineTool({
  ...
  handler: async ({ id }, ctx) => {
    ...
    const [dot, hm] = await Promise.all([
      ...
      sb.from("dot_bao_duong_hang_muc").select("id, he_thong_id, don_vi_id, trang_thai, ket_qua, nguon, han_hoan_thanh").eq("dot_bao_duong_id", id).limit(500),
    ]);
...
export const listBaoTri = defineTool({
  ...
  handler: async ({ he_thong_id, don_vi_id, tu_ngay, limit }, ctx) => {
    ...
    let q = supabaseForUser(ctx)
      .from("bao_tri")
      .select("id, ma_bao_tri, mo_ta_cong_viec, loai_bao_tri, he_thong_id, thiet_bi_id, don_vi_id, ngay_bat_dau, ngay_hoan_thanh, ket_qua")
...
export const listSuCo = defineTool({
  ...
  handler: async ({ trang_thai, muc_do, he_thong_id, tu_ngay, limit }, ctx) => {
    ...
    let q = supabaseForUser(ctx)
      .from("su_co")
      .select("id, ma_su_co, hien_tuong, muc_do, trang_thai, he_thong_id, thiet_bi_id, don_vi_id, ngay_phat_hien, nguoi_bao_cao")
...
export const listHongHoc = defineTool({
  ...
  handler: async ({ thiet_bi_id, tu_ngay, limit }, ctx) => {
    ...
    let q = supabaseForUser(ctx)
      .from("hong_hoc")
      .select("id, ma_hong_hoc, mo_ta_hong_hoc, bo_phan_hong, thiet_bi_id, ngay_hong, phuong_an, thiet_bi_thay_the_id")
...
export const listKhoGiaoDich = defineTool({
  ...
  handler: async ({ kho_id, limit }, ctx) => {
    const auth = requireAuth(ctx); if (auth) return auth;
    let q = supabaseForUser(ctx).from("kho_giao_dich").select("*").order("created_at", { ascending: false }).limit(limit);
...
export const metricTimeseries = defineTool({
  ...
  description: "Đọc v_metric_timeseries: chỉ số bảo dưỡng (metric_key) của hệ thống theo thời gian.",
  inputSchema: {
    he_thong_id: z.string().uuid().optional(),
  ...
  handler: async ({ he_thong_id, metric_key, tu_ngay, den_ngay, limit }, ctx) => {
    const auth = requireAuth(ctx); if (auth) return auth;
    let q = supabaseForUser(ctx).from("v_metric_timeseries").select("*").order("ts", { ascending: false }).limit(limit);
    if (he_thong_id) q = q.eq("he_thong_id", he_thong_id);
