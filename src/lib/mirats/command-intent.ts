...
export function matchIntent(input: string): Intent {
  ...
  // mount
  const mMount = raw.match(
    new RegExp(
      `(?:gán|gan|lắp|lap|mount)\\s+${ASSET_RE}\\s+(?:vào|vao|to|into)\\s+${COMP_RE}`,
      "i",
    ),
  );
  if (mMount)
    return {
      kind: "mount-asset",
      asset: mMount[1].toUpperCase(),
      component: mMount[2].toUpperCase(),
      confidence: 0.95,
    };

  // unmount
  const mUnmount = raw.match(
    new RegExp(
      `(?:tháo|thao|gỡ|go|unmount)\\s+${ASSET_RE}(?:\\s+(?:khỏi|khoi|from)\\s+${COMP_RE})?`,
      "i",
    ),
  );
  if (mUnmount)
    return {
      kind: "unmount-asset",
      asset: mUnmount[1].toUpperCase(),
      component: mUnmount[2]?.toUpperCase(),
      confidence: 0.9,
    };

  // close incident
  const mClose = raw.match(
    new RegExp(`(?:đóng|dong|close)\\s+(?:sự\\s+cố|su\\s+co|incident|sc)?\\s*${INCIDENT_RE}`, "i"),
  );
...
export function describeIntent(intent: Intent): string {
  switch (intent.kind) {
    case "mount-asset":
      return `Gán tài sản ${intent.asset} vào thành phần ${intent.component}`;
    case "unmount-asset":
      return intent.component
        ? `Tháo tài sản ${intent.asset} khỏi thành phần ${intent.component}`
        : `Tháo tài sản ${intent.asset}`;
    case "close-incident":
      return `Đóng sự cố ${intent.id}`;
    case "create-pm":
      return `Tạo phiếu bảo trì cho ${intent.target}`;
    case "logout":
      return "Đăng xuất khỏi hệ thống";
    case "navigate":
      return `Đi tới ${intent.label}`;
    case "jump-to":
      return `Tìm kiếm "${intent.query}"`;
    default:
      return "Hành động AI không xác định";
  }
}
