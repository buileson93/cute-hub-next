...
function bt(over: Partial<BaoTri> = {}): BaoTri {
  return {
    ma_bao_tri: "BT-1", thiet_bi: "TB-1", thiet_bi_id: "id-1", he_thong: "HT",
    he_thong_id: null, don_vi: "DV", loai_bao_tri: "Định kỳ", ke_hoach: null,
    ngay_bat_dau: "2026-01-10", ngay_hoan_thanh: null, mo_ta_cong_viec: "Vệ sinh",
    ket_qua: "Tốt", chi_phi: 0, nguoi_thuc_hien: [], don_vi_thuc_hien: "", trang_thai: "Hoàn thành",
    file_bien_ban: null, ...over,
  };
}
function sc(over: Partial<SuCo> = {}): SuCo {
  return {
    ma_su_co: "SC-1", thiet_bi: "TB-1", thiet_bi_id: "id-1", he_thong: "HT", he_thong_id: null,
    don_vi: "DV", ngay_phat_hien: "2026-02-15", nguoi_bao_cao: "", muc_do: "Nghiêm trọng",
    anh_huong_dhb: "", hien_tuong: "Mất tín hiệu", nguyen_nhan: null, bien_phap_xu_ly: "Khởi động lại",
    thoi_diem_khac_phuc: null, thoi_gian_gian_doan: null, nguoi_xu_ly: [], trang_thai: "Đã xử lý",
    lien_ket_hong_hoc: null, file_dinh_kem: null, bao_cao_ban_dau: null, ma_nhom_bc: null, van_de_id: null, ...over,
  };
}
function hh(over: Partial<HongHocThayThe> = {}): HongHocThayThe {
  return {
    id: "hh-1", thanh_phan_id: null,
    ma_hong_hoc: "HH-1", thiet_bi_hong: "TB-1", thiet_bi_hong_id: "id-1", su_co: null,
    ngay_hong: "2026-03-01", bo_phan_hong: "Nguồn", mo_ta_hong_hoc: "Cháy nguồn", phuong_an: "Thay mới",
    thiet_bi_thay_the: null, thiet_bi_thay_the_id: null, vat_tu_su_dung: [], chi_phi: 0,
    nguoi_thuc_hien: [], don_vi_thuc_hien: "", ket_qua: null, ngay_hoan_thanh: null,
    trang_thai: "Hoàn thành", file_dinh_kem: null, ...over,
  };
}
function bg(over: Partial<BanGiao> = {}): BanGiao {
  return {
    ma_ban_giao: "BG-1", thiet_bi: "TB-1", loai_ban_giao: "Điều chuyển", nguoi_giao: "A",
    nguoi_nhan: "B", don_vi_nhan: "DV2", ngay_nhan: "2026-04-20", ngay_tra: null,
    tinh_trang_khi_nhan: "Tốt", tinh_trang_khi_tra: null, file_bien_ban: null,
    trang_thai: "Hoàn thành", ghi_chu: null, ...over,
  };
}
...
describe("buildRecordTimeline", () => {
  it("gộp tất cả nguồn và sắp xếp mới nhất trước", () => {
    const items = buildRecordTimeline({
      baoTri: [bt()], suCo: [sc()], hongHoc: [hh()], banGiao: [bg()],
    });
  ...
  it("thêm sự kiện chỉnh sửa dữ liệu", () => {
    const items = buildRecordTimeline({
      changeEvents: [
        { at: "2026-05-01", action: "update", userName: "Admin", changesCount: 2, changesText: "A: x → y" },
  ...
  it("gắn nhãn tài sản: ưu tiên live rồi fallback snapshot", () => {
    ...
    const items = buildRecordTimeline({
      ...
      suCo: [sc({ thiet_bi_id: "id-gone", snapshot_ma_thiet_bi: "TB-OLD", snapshot_ten_thiet_bi: "Máy cũ" })],
