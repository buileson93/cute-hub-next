...
export function validateTemplate(tpl: DesignerTpl, fields: DesignerField[]): ValidationIssue[] {
  ...
  if (!tpl.ten.trim())
    out.push({ level: "error", field_index: null, message: "Tên mẫu không được trống." });
  ...
  fields.forEach((f, i) => {
    ...
    if (!label)
      out.push({
        ...
        field_index: i,
        field_key: key,
        message: "Thiếu nhãn hiển thị (label).",
      });
    ...
    if (KIND_NEEDS_OPTIONS.has(f.kind) && (!f.options || f.options.length === 0)) {
      out.push({
        ...
        field_index: i,
        field_key: key,
        message: `Trường "${label || key}" (${f.kind}) cần ít nhất 1 tuỳ chọn.`,
      });
    }
    if (KIND_NEEDS_COLUMNS.has(f.kind)) {
      ...
      if (cols.length === 0)
        out.push({
          ...
          field_index: i,
          field_key: key,
          message: `Trường "${label || key}" cần ít nhất 1 cột con.`,
        });
      ...
      cols.forEach((c, ci) => {
        if (!c.key?.trim() || !c.label?.trim())
          out.push({
            ...
            field_index: i,
            field_key: key,
            message: `Cột #${ci + 1} thiếu key hoặc label.`,
          });
        else if (seenCol.has(c.key))
          out.push({
            level: "error",
            field_index: i,
            field_key: key,
            message: `Cột "${c.key}" trùng lặp.`,
          });
        seenCol.add(c.key);
      });
    }
    ...
    if (f.min_value != null && f.max_value != null && f.min_value > f.max_value) {
      out.push({
        ...
        field_index: i,
        field_key: key,
        message: `min_value (${f.min_value}) > max_value (${f.max_value}).`,
      });
    }
    ...
    if (cs < 1 || cs > 3)
      out.push({
        ...
        field_index: i,
        field_key: key,
        message: `col_span nên nằm trong [1..3] (hiện tại ${cs}).`,
      });
    ...
    for (const rule of [
      ...
      if (rule.v && !knownKeys.has(rule.v.field_key)) {
        out.push({
          ...
          field_index: i,
          field_key: key,
          message: `${rule.name} tham chiếu field_key "${rule.v.field_key}" không tồn tại.`,
        });
      }
      if (rule.v && rule.v.field_key === key) {
        out.push({
          ...
          field_index: i,
          field_key: key,
          message: `${rule.name} không được tham chiếu chính nó.`,
        });
      }
    }
    ...
    if (f.constraint_formula && !f.constraint_message?.trim()) {
      out.push({
        ...
        field_index: i,
        field_key: key,
        message: `Có constraint_formula nhưng thiếu constraint_message hiển thị cho người dùng.`,
      });
    }
  });
...
export async function persistDesigner(
  templateId: string,
  tpl: DesignerTpl,
  fields: DesignerField[],
  linkedHt: string[],
  ...
  const { error: delLink } = await supabase
  ...
  if (links.length > 0) {
    const { error: insLink } = await supabase
      .from("form_template_he_thong")
