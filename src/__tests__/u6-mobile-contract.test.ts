...
describe("u6-mobile-contract: Mobile Integrity Guard", () => {
  ...
  it("không làm tăng rủi ro về chiều rộng (min-w, w-[...px])", () => {
    ...
    const diff =
      cRisks.minWidthPx + cRisks.fixedWidthLarge - (bRisks.minWidthPx + bRisks.fixedWidthLarge);
    expect(
      diff,
      `Tổng số rủi ro chiều rộng (min-w, fixed w) tăng thêm ${diff}. Hạn chế hardcode pixel cho chiều rộng.`,
  ...
  it("không làm tăng số lượng grid-cols không có tiền tố responsive", () => {
    ...
    expect(
      diff,
      `Số lượng grid-cols không có prefix responsive tăng thêm ${diff}. Hãy dùng sm:grid-cols-...`,
    ).toBeLessThanOrEqual(0);
  });
  ...
  it("không làm tăng số lượng đích chạm nhỏ (h-6, h-7, h-8)", () => {
    ...
    expect(
      diff,
      `Số lượng đích chạm nhỏ (h-6/7/8) tăng thêm ${diff}. Ưu tiên h-11 trên mobile.`,
    ).toBeLessThanOrEqual(0);
  });
  ...
  it("tất cả các route hạng G3 phải được bao bọc bởi DesktopOnly", () => {
    ...
    expect(
      violations,
      `Các route G3 sau chưa được bao bởi <DesktopOnly>:\n${violations.join("\n")}`,
    ).toEqual([]);
  });
